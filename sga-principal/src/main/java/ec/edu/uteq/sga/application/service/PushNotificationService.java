...
public class PushNotificationService {
 ...
 private Map<String,String> datos(EventoAcademicoPushDTO d,Usuario u){Map<String,String> m=new HashMap<>();m.put("type",d.type()); if(d.announcementId()!=null)m.put("announcementId",d.announcementId().toString()); if(d.periodId()!=null)m.put("periodId",d.periodId().toString()); if(d.studentName()!=null)m.put("studentName",d.studentName()); if(d.date()!=null)m.put("date",d.date().toString()); if(d.attendanceId()!=null)m.put("attendanceId",d.attendanceId().toString()); if(d.matriculaId()!=null)matriculas.findById(d.matriculaId()).ifPresent(x->m.put("studentId",x.getEstudiante().getIdEstudiante().toString()));return m;}
}
