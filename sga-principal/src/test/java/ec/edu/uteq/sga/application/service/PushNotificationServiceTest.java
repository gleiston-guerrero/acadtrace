...
class PushNotificationServiceTest {
 ...
 @Test void ausenciaGeneraUnaSolaVezYDesactivaTokenInvalido(){
  ...
  EventoAcademicoPushDTO dto=new EventoAcademicoPushDTO("ASISTENCIA:8:AUSENTE","AUSENTE",7L,null,3L,null,null,"Ana Paz registro una ausencia",date,"Ana Paz",8L);
  ...
  service.procesar(dto); assertThat(device.isActivo()).isFalse(); verify(fcm).send(eq("oculto"),eq("Ausencia registrada"),anyString(),argThat(data -> "AUSENTE".equals(data.get("type")) && "5".equals(data.get("studentId")) && "Ana Paz".equals(data.get("studentName")) && "2026-09-11".equals(data.get("date")) && "8".equals(data.get("attendanceId"))));
 }
 @Test void eventoYaEnviadoNoDuplica(){
  EventoAcademicoPushDTO dto=new EventoAcademicoPushDTO("k","ATRASO",7L,null,null,null,null,"real",null,null,null);
 ...
 @Test void falloFcmQuedaRegistradoSinLanzarExcepcion(){
  EventoAcademicoPushDTO dto=new EventoAcademicoPushDTO("fallo","AUSENTE",7L,null,null,null,null,"real",null,null,null);
