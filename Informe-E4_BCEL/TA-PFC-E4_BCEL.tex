...
\caption{Resultados de la prueba de carga con Locust ($12{,}265$ peticiones con 0\,\% tasa de error bajo escenario nominal de 5 minutos; $212$ peticiones en corrida preliminar de calibración).}
...
\hline
\textbf{2. Rendimiento} \newline \textit{(Performance)} & \textbf{Nominal: $P_{95} = 370$\,ms, $MD = 110$\,ms} \newline (Throughput $41.0$\,RPS en Esc-1; latencia de BD $P_{95} = 400$\,ms en $3{,}428$ reqs) & $< 500$\,ms & \textbf{\checkmark} Cumple & Integrar una capa de almacenamiento en caché distribuida en memoria mediante Redis para catálogos y mallas curriculares, reduciendo latencias a $< 15$\,ms. \\
\hline
...
\hline
\textbf{4. Mantenibilidad} \newline \textit{(Maintainability)} & \textbf{30.31\% SGA Principal / 34.52\% Sec} \newline (100\% en lógica de negocio \texttt{AuditoriaService} y \texttt{LamportClock}; 30.31\% global del módulo) & $\ge 70.0\%$ & Parcial (30.31\%) & Ampliar progresivamente la suite de pruebas unitarias hacia controladores REST y adaptadores de infraestructura para alcanzar la meta global del 70\%. \\
\hline
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Juliana Romina Emanuel Pino (Responsable de Microservicio Soporte y Observabilidad):} En esta entrega se completó la refactorización del microservicio de Soporte y Gestión de Tickets bajo Java 17/Spring Boot, incorporando el bus de comunicación gRPC (elección de líder vía \texttt{etcd}) y la arquitectura de observabilidad con Prometheus, Grafana y cAdvisor. El diseño del tablero de Grafana (\texttt{ops/grafana/pfc-dashboard.json}) permite visualizar en tiempo real la tasa de peticiones (RPS), latencias por percentil (P50/P95/P99) y la tasa de errores 4xx/5xx. En la prueba de carga oficial ejecutada con Locust bajo el escenario nominal (50 usuarios concurrentes durante 5 minutos, autenticación JWT generada localmente sin dependencia externa) se procesaron 12{,}265 peticiones con 0\,\% de tasa de error (40.96\,RPS promedio; una corrida preliminar de calibración registró 212 peticiones con 0\,\% de fallos), observándose una disparidad de latencia medible entre endpoints: \texttt{/health} y \texttt{/actuator/health} respondieron con mediana de 110\,ms y P95 de 140\,ms, mientras que \texttt{/api/soporte/tickets} (dependiente de base de datos) promedió 400.8\,ms con mediana de 320\,ms y P95 de 400\,ms en 3{,}428 transacciones exitosas. Un hallazgo metodológico relevante fue evidenciar que la resiliencia observada bajo carga sostenida depende de la correcta provisión de secretos y de la arquitectura de sus dependencias de base de datos, reforzando la práctica de gestión segura de secretos mediante variables de entorno y eliminando por completo cualquier token estático expuesto.
\end{itemize}
