...
\noindent\textbf{An\'alisis Estad\'istico Reproducible de Latencias:} La Tabla~\ref{tab:estadistica-inferencial} se genera directamente desde \texttt{experimentos/resultados/exp1\_concurrencia.csv}, usando las 40 observaciones de \texttt{latencia\_mediana\_ms} de cada mecanismo. La entrada y la salida se expresan en milisegundos (ms), sin conversi\'on adicional. Los IC 95\% de las medianas se obtienen mediante bootstrap no param\'etrico con $B=10{,}000$ y semilla fija 20260831. Como magnitud del efecto en unidades originales se reporta $\Delta$ mediana frente a M0, tambi\'en con IC 95\% bootstrap. No se interpreta como tama\~no de efecto estandarizado y no se publican contrastes $U$ ni valores $p$. En esta corrida los IC de M0 y M1 presentan amplitud cero por la concentraci\'on de las mediciones; esta limitaci\'on se conserva y declara.
...
\begin{enumerate}
  \item \textbf{Resoluci\'on temporal y entorno de ejecuci\'on:} Las 40 observaciones de latencia por mecanismo se conservaron completas; no se descartaron la primera ni la \'ultima repetici\'on. El bloque de latencias es un microbenchmark local secuencial en memoria, no una prueba de concurrencia HTTP ni de acceso real a base de datos. La resoluci\'on del temporizador y la planificaci\'on del sistema operativo constituyen amenazas residuales.
...
\textbf{Respuesta:} El bloque vigente de latencias es un microbenchmark local secuencial en memoria. No representa cincuenta solicitudes concurrentes, acceso real a base de datos ni una operaci\'on HTTP completa. La magnitud se reporta como diferencia de medianas frente a m0, expresada en milisegundos y acompa\~nada por un IC 95\% bootstrap desde \texttt{exp1\_concurrencia.csv}; por ello no se extrapola linealmente el resultado a cincuenta notas.
