...
\begin{enumerate}
  \item \textbf{Unitarias:} Evidencia hist\'orica conservada de pruebas de servicios con dobles de prueba (Mockito en Java, \texttt{pytest-mock} en Python): Cobertura parcial documentada en iteraciones previas, pendiente de consolidación final.
...
\subsection{Unificaci\'on Can\'onica y Sincronizaci\'on entre Microservicios Pol\'iglotas}
\label{subsec:unificacion-canonica}
La arquitectura distribuida de AcadTrace integra servicios desacoplados bajo diferentes lenguajes y plataformas de ejecuci\'on: \texttt{sga-principal} y \texttt{microservicio-secretaria} sobre Java 21 / Spring Boot 3, y \texttt{microservicio-docente} sobre Python 3.11 / Django REST Framework. Para garantizar que cualquier evento registrado por cualquiera de los tres servicios pertenezca a la misma cadena criptogr\'afica global sin requerir dependencias binarias compartidas entre runtimes heterog\'eneos, el contrato de auditor\'ia se sincroniza de manera estricta a trav\'es de un \textbf{vector patr\'on can\'onico} compuesto exactamente por once campos normalizados:
\begin{center}
\texttt{[\"accion\", \"app\", \"estado\_reconciliacion\", \"fecha\", \"lamport\", \"modo\_auditoria\", \"payload\", \"registro\_id\", \"tabla\", \"usuario\", \"vclock\"]}
\end{center}

Esta sincronizaci\'on se encuentra formalmente protegida y verificada contra derivas de implementaci\'on (\textit{schema drift}) mediante los siguientes mecanismos:
\begin{enumerate}[leftmargin=*]
  \item \textbf{Normalizaci\'on Can\'onica Determinista:} Cada runtime implementa serializaci\'on lexicogr\'afica ordenada alfab\'eticamente por clave sin espacios innecesarios y con codificaci\'on UTF-8 estricta (\texttt{AuditHashService.java} en Java y \texttt{auditoria\_service.py} en Python), asegurando representaci\'on id\'entica car\'acter por car\'acter.
  \item \textbf{Verificaci\'on Automatizada por Vector Patr\'on:} Bater\'ias de pruebas can\'onicas dedicadas (\texttt{AuditHashServiceCanonicalTest} en Java y \texttt{test\_auditoria\_canonica.py} en Python) eval\'uan el mismo vector can\'onico de 11 campos y verifican la coincidencia exacta del hash SHA-256 generado. La adici\'on, renombrado o alteraci\'on de cualquier campo rompe inmediatamente las aserciones de prueba en ambas plataformas en el pipeline de CI/CD.
  \item \textbf{Serializaci\'on Concurrente y Siembra de Reloj:} La inmutabilidad de la secuencia global y la prevenci\'on de bifurcaciones se sustentan f\'isicamente mediante bloqueo pesimista (\texttt{SELECT ... FOR UPDATE}) sobre la fila singleton (\texttt{id\_estado = 1}) de la tabla \texttt{sga\_principal.estado\_cadena\_auditoria}. Asimismo, los relojes l\'ogicos de Lamport y vectoriales se inicializan (\textit{seeding}) desde el \'ultimo estado persistido en base de datos, garantizando monoton\'ia estricta aun tras reinicios de las instancias.
\end{enumerate}
...
\noindent\textbf{An\'alisis Estad\'istico Reproducible de Latencias:} La Tabla~\ref{tab:estadistica-inferencial} se genera directamente desde \texttt{experimentos/resultados/exp1\_concurrencia.csv}, usando las 40 observaciones de \texttt{latencia\_mediana\_ms} de cada mecanismo. La entrada y la salida se expresan en milisegundos (ms), sin conversi\'on adicional. Los intervalos corresponden al bootstrap no param\'etrico de la mediana con $B=10{,}000$ y semilla fija 20260831. Se descartan los contrastes $U$, valores $p$ y tama\~nos de efecto hist\'oricos porque no pertenecen a esta fuente oficial.

\input{tabla_latencias_generada.tex}

\begin{figure}[H]
\centering
\includegraphics[width=0.78\textwidth]{boxplot_latencia.png}
\caption{Distribuci\'on de las 40 observaciones de \texttt{latencia\_mediana\_ms} por mecanismo, expresadas en milisegundos y obtenidas de \texttt{exp1\_concurrencia.csv}.}
...
\hline
\textbf{1. Disponibilidad} \newline \textit{(Availability)} & 0 fallos en 12{,}994 peticiones nominales oficiales (100\% de éxito en la muestra local); sin medición oficial válida de estrés & $\ge 99.5\%$ & Cumple en nominal & Desplegar clúster multi-zona de disponibilidad (Multi-AZ) con auto-escalado horizontal (HPA) en Kubernetes y réplicas primario-standby geodistribuidas para PostgreSQL para superar el cuello de botella monoinstancia. \\
\hline
...
\hline
\textbf{3. Fiabilidad} \newline \textit{(Reliability)} & 0 fallos en 12{,}994 peticiones (0.0\% tasa de fallos nominal oficial); sin estrés oficial validado & $<1.0\%$ & \textbf{\checkmark} Cumple en nominal & Implementar patrones \emph{Circuit Breaker} y \emph{Rate Limiting} con Resilience4j en el Gateway y clientes gRPC para mitigar fallos en cascada y aplicar degradación elegante ante sobrecargas severas. \\
\hline
\textbf{4. Mantenibilidad} \newline \textit{(Maintainability)} & Secretar\'ia: 71.43\% LINE (2220/3108) \newline Soporte: 71.61\% LINE (459/641) & $\ge 70\%$ LINE JaCoCo BUNDLE & \textbf{\checkmark} Cumple en Secretar\'ia y Soporte & Mantener la compuerta JaCoCo a nivel BUNDLE en CI/CD y expandir cobertura automatizada en SGA Principal y Docente. \\
\hline
