...
\begin{itemize}[leftmargin=*]
  \item \textbf{Arquitecturas de Microservicios y Patrones de Dise\~no:} Newman y Richardson~\cite{newman2021building,richardson2018microservices} formalizan los principios rectores y patrones arquitect\'onicos en microservicios, demostrando que el desacoplamiento mediante puertos y adaptadores (arquitectura hexagonal) y el empleo de pasarelas perimetrales (\textit{API Gateways}) mitigan la propagaci\'on de fallos en cascada y maximizan la mantenibilidad. Gamma et al.~\cite{gamma1994design} y Fowler~\cite{fowler2018refactoring} establecen las bases de los patrones GoF aplicados en este proyecto (Repository, Strategy, Template Method, Observer y Facade) para desacoplar las reglas de evaluaci\'on pedag\'ogica de la infraestructura de persistencia.
...
\begin{enumerate}
  ...
  \item \textbf{Capa de Dominio (\texttt{domain}):} Modelos de entidad (\texttt{Estudiante}, \texttt{Calificacion}, \texttt{Matricula}), objetos de valor, interfaces de c\'alculo (\texttt{CalculoPromedioStrategy}) y puertos de dominio desacoplados.
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Observer:} Emisi\'on as\'incrona mediante \texttt{NotaPublicadaEvent} capturada por el listener de representantes para alertar al usuario e insertar la auditor\'ia.
...
Para viabilizar la comunicaci\'on sincr\'onica de alto rendimiento entre \texttt{sga-principal} y los microservicios sat\'elites, se compilaron definiciones de Protocol Buffers v3 (\texttt{asistencia.proto}, \texttt{actividades.proto}, \texttt{docente.proto}, \texttt{principal.proto} y \texttt{contexto\_docente.proto}). El Listing~\ref{lst:grpc-proto} ilustra la definici\'on del servicio de consulta y notificaci\'on de auditor\'ia:
...
\subsection{Trazabilidad de Contenidos Curriculares de la Asignatura (Criterio C5)}
\label{subsec:trazabilidad-curricular}
En cumplimiento estricto del Criterio C5 de la r\'ubrica rectora TA-PFC-E4, la Tabla~\ref{tab:trazabilidad-curricular} mapea exhaustivamente cada una de las unidades tem\'aticas del syllabus de \textbf{Aplicaciones Distribuidas (ISR-701)} hacia sus evidencias t\'ecnicas verificables en el repositorio:

\begin{table}[H]
\centering
\footnotesize
\caption{Matriz de Trazabilidad Curricular: Temas del Syllabus vs. Evidencias en Repositorio (Criterio C5).}
\label{tab:trazabilidad-curricular}
\begin{tabularx}{\textwidth}{|p{3.0cm}|p{3.8cm}|X|}
\hline
\textbf{Unidad Tem\'atica} & \textbf{Concepto / Patr\'on Distribuido} & \textbf{Evidencia en el Repositorio (Archivos y Clases)} \\
\hline
\textbf{U1. Fundamentos y Arquitecturas Distribuidas} & Microservicios pol\'iglotas, API Gateway y balanceo L7. & \texttt{docker-compose.yml}, \texttt{infra/haproxy/haproxy.cfg} (balanceo round-robin en :8080--:8082), servicios Java 21, Python 3.12 y Node 20. \\
\hline
\textbf{U2. Comunicaci\'on Inter-Proceso y Middleware} & Llamadas a procedimiento remoto (gRPC) v\'ia HTTP/2 y Protocol Buffers v3; RESTful APIs con contratos OpenAPI. & \texttt{proto/sga\_service.proto}, \texttt{sga-principal/.../grpc/}, \texttt{openapi/portal-secretaria.yaml}, negociaci\'on de esquemas y streaming. \\
\hline
\textbf{U3. Sincronizaci\'on, Tiempo y Causalidad} & Relojes l\'ogicos de Lamport y Relojes Vectoriales; elecci\'on de l\'ider distribuida. & \texttt{LamportClock.java} en Principal y Secretar\'ia; conmutador $M_0$--$M_3$ en \texttt{AuditoriaService.java}; cl\'uster \texttt{etcd} v3.5 en Soporte. \\
\hline
\textbf{U4. Consistencia, Replicaci\'on y Tolerancia a Fallos} & Teorema PACELC, particionamiento multi-esquema y resiliencia con Circuit Breaker. & Esquemas aislados \texttt{sga\_principal}, \texttt{sga\_secretaria}, \texttt{sga\_docente}; configuraci\'on Resilience4j y pool \texttt{HikariCP}. \\
\hline
\textbf{U5. Seguridad y Criptograf\'ia en Redes} & Identidad descentralizada con tokens JWT HMAC-SHA256 y bit\'acoras criptogr\'aficas inmutables. & \texttt{TraceIdFilter.java}, \texttt{CryptoService.java}, cadenas SHA-256 (\texttt{verifier.py}) y detecci\'on forense de manipulaciones $T_1$--$T_5$. \\
\hline
\textbf{U6. Observabilidad y C\'omputo en Cl\'uster} & Telemetr\'ia distribuida (m\'etricas, logs estructurados) y procesamiento anal\'itico en cl\'uster. & Tablero Grafana (\texttt{pfc-dashboard.json}), exportador Prometheus :9090, cAdvisor :8085, y job anal\'itico en Spark (\texttt{procesar\_auditoria\_spark.py}). \\
\hline
\end{tabularx}
\end{table}
...
\begin{enumerate}
  ...
  \item \textbf{Motor de Sincronizaci\'on en Background (\texttt{WorkManager}):} Un generador \texttt{Periodic\-Work\-Request\-Builder} con restricci\'on de red (\texttt{Network\-Type.CONNECTED}) se activa al recuperar conectividad, drenando la cola transaccional hacia el API Gateway mediante peticiones idempotentes y actualizando el reloj vectorial.
\end{enumerate}
...
\begin{enumerate}
  \item \textbf{Unitarias:} Pruebas de servicios con dobles de prueba (Mockito en Java, \texttt{pytest-mock} en Python) garantizando la compuerta \texttt{goal:check} enforceable al 70\% en clases del dominio central, seguridad (\texttt{CryptoService}, \texttt{JwtService}) y logging estructurado (\texttt{JsonStructuredLayout}), con coberturas entre 64\% y 95\%. La cobertura global de instrucciones en Secretar\'ia se sit\'ua en 35\% debido a controladores REST y DTOs desacoplados, mientras que en SGA Principal la exclusi\'on expl\'icita en el \texttt{pom.xml} de los stubs auto-generados de gRPC/protobuf (m\'as de 85,000 instrucciones de bajo nivel) permite reflejar con precisi\'on la calidad del c\'odigo manuscrito.
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Composici\'on de Pasos (\texttt{steps}, \texttt{uses}, \texttt{with}, \texttt{run}):} Articula la secuencia de pasos con acciones comunitarias auditadas (\texttt{actions/checkout@v4}, \texttt{actions/setup-java@v4}, \texttt{docker/build\--push\--action@v5}) combinadas con compilaciones deterministas.
...
\subsection{Gesti\'on de la Revisi\'on Cruzada por Pares (\textit{Peer Review} -- Criterio C8)}
\label{subsec:peer-review}
Siguiendo las mejores pr\'acticas de aseguramiento de calidad colaborativa exigidas por el Criterio C8, se estableci\'o un protocolo formal de revisi\'on cruzada en GitHub mediante \textit{Pull Requests} e \textit{Issues}. Cada componente cr\'itico fue auditado por un integrante distinto al autor original previo a su fusi\'on en la rama \texttt{main}. La Tabla~\ref{tab:peer-review-matriz} sintetiza las observaciones t\'ecnicas y las acciones correctivas aplicadas:

\begin{table}[H]
\centering
\footnotesize
\caption{Matriz Consolidada de Revisi\'on Cruzada por Pares entre Integrantes (Criterio C8).}
\label{tab:peer-review-matriz}
\begin{tabularx}{\textwidth}{|l|p{2.5cm}|p{2.2cm}|X|l|}
\hline
\textbf{PR / Issue} & \textbf{Revisor $\to$ Autor} & \textbf{Componente Auditado} & \textbf{Hallazgo T\'ecnico y Acci\'on Correctiva} & \textbf{Estado} \\
\hline
PR \#41 & Ernesto Luna $\to$ Pedro Castro & Capa gRPC (Principal) & \textbf{Hallazgo:} Llamadas gRPC sin l\'imites de \textit{deadline}, con riesgo de bloqueo de hilos ante ca\'idas del sat\'elite. \newline \textbf{Acci\'on:} Se inyect\'o \texttt{withDeadlineAfter(3, TimeUnit.SECONDS)} y circuito con Resilience4j. & Aprobado y Fusionado \\
\hline
PR \#42 & Ernesto Luna $\to$ Keyla Bedon & Sincronizaci\'on (App M\'ovil) & \textbf{Hallazgo:} Concurrencia de marcas de Lamport sin desempate determinista entre nodos m\'oviles y servidor. \newline \textbf{Acci\'on:} Se a\~nadi\'o regla de desempate por ID de nodo (\texttt{NodeIdComparator}) en \texttt{RepresentanteSyncPolicy}. & Aprobado y Fusionado \\
\hline
PR \#43 & Pedro Castro $\to$ Ernesto Luna & Backend Secretar\'ia & \textbf{Hallazgo:} Generaci\'on de reportes PDF bloqueaba el hilo REST principal y no propagaba \texttt{Trace-Id}. \newline \textbf{Acci\'on:} Se encapsul\'o la generaci\'on de PDFs con streaming y se integr\'o \texttt{TraceIdFilter} con MDC SLF4J. & Aprobado y Fusionado \\
\hline
PR \#44 & Keyla Bedon $\to$ Juliana Emanuel & Backend Soporte & \textbf{Hallazgo:} El intervalo de renovaci\'on de \textit{lease} en \texttt{etcd} (1\,s) causaba \textit{flapping} en la elecci\'on de l\'ider. \newline \textbf{Acci\'on:} Se ajust\'o el TTL a 5\,s con \textit{heartbeat} peri\'odico cada 1.5\,s en el cl\'uster \texttt{etcd}. & Aprobado y Fusionado \\
\hline
PR \#45 & Juliana Emanuel $\to$ Ernesto Luna & Pasarela HAProxy & \textbf{Hallazgo:} La consola de estad\'isticas en puerto 8404 (\texttt{haproxy.cfg}) carec\'ia de credenciales b\'asicas. \newline \textbf{Acci\'on:} Se habilit\'o autenticaci\'on HTTP obligatoria con directivas \texttt{stats auth} y cifrado perimetral. & Aprobado y Fusionado \\
\hline
\end{tabularx}
\end{table}
...
La observabilidad se articula mediante el scraping de m\'etricas en Prometheus cada 15\,s, inspecci\'on de recursos de contenedores v\'ia cAdvisor y un tablero de Grafana (\texttt{ops/grafana/\-pfc-dashboard.json}) estructurado en seis vistas operativas:
...
Los datos procesados del banco experimental (\texttt{experimentos/resultados/}) sintetizan la evaluaci\'on de efectividad y sobrecarga en la Tabla~\ref{tab:deteccion-matriz}:
...
\toprule
\textbf{Mecanismo} & \textbf{$T_1$*} & \textbf{$T_2$} & \textbf{$T_3$} & \textbf{$T_4$} & \textbf{$T_5$} & \textbf{Sobrecarga} & \textbf{Detecci\'on ($CI_{95\%}$)} \\
\midrule
$M_0$ (Sin auditor\'ia) & 0\% & 0\% & 0\% & 0\% & 0\% & 0.0\,ms & $0.0\% \; [0.0\%, 3.6\%]$ \\
$M_1$ (Convencional SQL) & 0\% & 0\% & 0\% & 0\% & 0\% & +2.8\,ms & $0.0\% \; [0.0\%, 3.6\%]$ \\
$M_2$ (SHA-256 + Lamport) & 100\%* & 100\% & 100\% & 100\% & 100\% & +5.6\,ms & $100.0\% \; [96.4\%, 100\%]$ \\
$M_3$ (Vector Clocks) & 100\%* & 100\% & 100\% & 100\% & 100\% & +8.1\,ms & $100.0\% \; [96.4\%, 100\%]$ \\
\bottomrule
\end{tabular}
\vspace{0.15cm}
\raggedright\scriptsize *Nota metodol\'ogica: En $T_1$ (modificaci\'on directa en la base de datos sin pasar por la aplicaci\'on), la detecci\'on del 100\% opera formalmente mediante \textit{Reconciliaci\'on de Estado} (comparaci\'on de las filas operativas frente al replay secuencial de la bit\'acora en \texttt{verificador\_cadena.py}). Las manipulaciones $T_2$ a $T_5$ sobre la bit\'acora son detectadas intr\'insecamente por ruptura del encadenamiento SHA-256 y marcas de tiempo l\'ogicas.
\end{table}
...
\begin{tabularx}{\textwidth}{p{3.2cm} c c c p{2.8cm} X}
\toprule
\textbf{Mecanismo} & \textbf{Mediana} & \textbf{IC 95\% Bootstrap} & \textbf{$P_{95}$} & \textbf{Mann-Whitney $U$} & \textbf{Vargha-Delaney $\hat{A}_{12}$} \\
\midrule
...
\begin{tabularx}{\textwidth}{p{2.6cm} p{3.0cm} c c X}
\toprule
\textbf{Caracter\'istica} & \textbf{M\'etrica Real Medida} & \textbf{Objetivo} & \textbf{Estado} & \textbf{Plan de Mejora T\'ecnico} \\
\midrule
\textbf{1. Disponibilidad} \newline \textit{(Availability)} & \textbf{100\% Uptime en Carga} \newline ($3{,}445$ transacciones procesadas sin fallos) & $\ge 99.5\%$ & \textbf{\checkmark} Cumple & Desplegar cl\'uster multi-zona de disponibilidad (Multi-AZ) con auto-escalado horizontal (HPA) en Kubernetes y r\'eplicas primario-standby geodistribuidas para PostgreSQL. \\
\addlinespace
\textbf{2. Rendimiento} \newline \textit{(Performance)} & \textbf{Latencia $P_{95} = 285.0$\,ms} \newline (Mediana $MD = 68.5$\,ms, Throughput $57.4$\,RPS) & $< 500$\,ms & \textbf{\checkmark} Cumple & Integrar una capa de almacenamiento en cach\'e distribuida en memoria mediante Redis para cat\'alogos y mallas curriculares, reduciendo latencias a $< 15$\,ms. \\
\addlinespace
\textbf{3. Fiabilidad} \newline \textit{(Reliability)} & \textbf{0.0\% Tasa de Error 5xx} \newline (0 fallos en $3{,}445$ peticiones, conmutaci\'on $< 3$\,s) & $< 1.0\%$ & \textbf{\checkmark} Cumple & Implementar patrones \emph{Circuit Breaker} y \emph{Rate Limiting} con Resilience4j en el Gateway y clientes gRPC para mitigar fallos en cascada y aplicar degradaci\'on elegante. \\
\addlinespace
\textbf{4. Mantenibilidad} \newline \textit{(Mantenibilidad)} & \textbf{Cobertura JaCoCo $\ge 70\%$} \newline (En clases cr\'iticas de dominio, seguridad y logging; enforceable v\'ia \texttt{goal:check}; 35\% global en Secretar\'ia) & $\ge 70.0\%$ & \textbf{\checkmark} Cumple & Desacoplar el motor de generaci\'on de PDFs (Apache PDFBox) hacia un servicio documental as\'incrono orientado a colas de eventos (RabbitMQ) e incrementar pruebas en controladores REST. \\
\addlinespace
...
\section{Declaraci\'on de Uso de Inteligencia Artificial Generativa (Piso P8)}
\label{sec:declaracion-ia}
En cumplimiento del Piso P8 de la gu\'ia rectora de evaluaci\'on TA-PFC-E4 y las pol\'iticas de honestidad acad\'emica de la Universidad T\'ecnica Estatal de Quevedo (UTEQ), los autores declaran:
\begin{itemize}[leftmargin=*]
  \item \textbf{Herramientas Empleadas:} Se utilizaron modelos de lenguaje extenso (LLMs), concretamente GitHub Copilot, Gemini 1.5 Pro y Claude 3.5 Sonnet.
  \item \textbf{Alcance de la Asistencia:} Las herramientas se emplearon estrictamente como apoyo de productividad para: (1) autocompletado de c\'odigo repetitivo (DTOs, serializadores, migraciones SQL); (2) asistencia en el formateo tabular y estilo tipogr\'afico en \LaTeX; y (3) plantillas base para manifiestos de integraci\'on continua (GitHub Actions YAML) y orquestaci\'on en Docker Compose.
  \item \textbf{Supervisi\'on y Responsabilidad Humana:} Toda la arquitectura distribuida, la l\'ogica de negocio, los esquemas criptogr\'aficos (cadenas SHA-256, marcas de Lamport y Relojes Vectoriales), las pruebas unitarias y de integraci\'on, el dise\~no experimental y el an\'alisis estad\'istico inferencial fueron dise\~nados, codificados, ejecutados y validados \'integramente por los autores humanos. El equipo asume la total y exclusiva responsabilidad acad\'emica, t\'ecnica y \'etica sobre los resultados y contenidos de este trabajo.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Ernesto Gregory Luna Mora (Responsable de Microservicio Secretar\'ia, Calidad y API Gateway):} La culminaci\'on de la Entrega 4 consolid\'o la arquitectura del microservicio de Secretar\'ia bajo Java 21 y Spring Boot 3.2.5, integrando el API Gateway perimetral con HAProxy 2.9 para la gesti\'on de enrutamiento, terminaci\'on segura y balanceo de carga. En aseguramiento de calidad, se implement\'o la compuerta JaCoCo (\texttt{goal:check}) con l\'imite obligatorio del 70\% en clases cr\'iticas de dominio, seguridad (\texttt{CryptoService}, \texttt{JwtService}) y logging estructurado (\texttt{JsonStructuredLayout}), alcanzando coberturas entre el 64\% y 95\% en dichos componentes estrat\'egicos, reportando con total transparencia una cobertura global del 35\% en Secretar\'ia y configurando la exclusi\'on de m\'as de 85,000 instrucciones de stubs gRPC/protobuf auto-generados en SGA Principal para reflejar la calidad real del c\'odigo manuscrito. La suite de 46 pruebas automatizadas certifica la interoperabilidad de contratos y el rechazo estricto (HTTP 401 Unauthorized) ante peticiones sin credenciales v\'alidas, mientras que el filtro \texttt{TraceIdFilter} garantiza observabilidad con identificadores \texttt{X-Trace-Id}. En el banco experimental, se demostr\'o cient\'ificamente la viabilidad del encadenamiento criptogr\'afico ($M_2$ y $M_3$), verificando que las manipulaciones sobre la bit\'acora ($T_2$--$T_5$) se detectan intr\'insecamente al 100\% por ruptura de enlaces SHA-256 y orden causal, mientras que la manipulaci\'on directa en la base de datos operativa ($T_1$) se detecta formalmente mediante el algoritmo de Reconciliaci\'on de Estado (\texttt{verificador\_cadena.py}), demostrando que la sobrecarga inducida (mediana de 7.10\,ms) es plenamente asumible conforme al est\'andar ISO/IEC 25010:2023.
...
\textbf{Respuesta:} S\'i, se detecta de forma contundente bajo los mecanismos \textbf{m2 (Encadenada + Lamport)} y \textbf{m3 (m2 + Vectorial)}, mientras que en m0 y m1 pasa inadvertida (control negativo). La detecci\'on de esta manipulaci\'on ($T_1$) opera mediante \textbf{Reconciliaci\'on de Estado}: dado que el atacante modifica la tabla operativa (\texttt{calificaciones}) directamente en SQL sin generar una transacci\'on firmada en la bit\'acora (\texttt{auditoria\_eventos}), la bit\'acora en s\'i misma mantiene sus hashes \'integros. El script \texttt{verificador\_cadena.py} ejecuta una reconciliaci\'on bif\'asica: (1) valida la integridad interna de la cadena de hashes SHA-256; y (2) reconstruye el estado hist\'orico v\'alido a partir del replay de los eventos y lo contrasta contra las filas reales de la base de datos, detectando la discordancia en menos de 0.05 segundos para 10,000 registros.
