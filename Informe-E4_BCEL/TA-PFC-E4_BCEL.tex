...
La Tabla~\ref{tab:iso-matriz-calidad} presenta la matriz formal de evaluaci\'on de calidad ISO/IEC 25010 generada de forma reproducible a partir de la evidencia experimental auditada (\texttt{matriz\_iso25010\_generada.tex}). El alcance se delimita a la corrida nominal local oficial del Microservicio de Soporte (Conjunto~A, con \CargaPeticiones{} peticiones y \CargaFallos{} fallos observados a \CargaUsuariosMaximos{} usuarios concurrentes). Conforme a este alcance y a la respuesta emp\'irica de PI-1, la latencia agregada $P_{99}=\CargaPnoventaNueveMs$\,ms no cumple el umbral estricto ($P_{99}<500$\,ms). Las 12{,}994 peticiones con 0 fallos constituyen evidencia parcial de la ejecuci\'on nominal y no demuestran fiabilidad global, disponibilidad de producci\'on ni un comportamiento de estr\'es exitoso. En mantenibilidad, se acredita el cumplimiento del umbral de cobertura en Secretar\'ia (71.42\,\%) y Soporte (71.61\,\%), mientras que Principal (31.6\,\%) se encuentra en curso. Las caracter\'isticas de adecuaci\'on funcional, seguridad, usabilidad, portabilidad y compatibilidad se registran con el veredicto expl\'icito de \textit{No evaluado} al no existir evidencia reproducible auditada dentro de los l\'imites de esta matriz; dicho veredicto no significa que el sistema incumpla, sino que la evidencia disponible no permite emitir ese dictamen:
