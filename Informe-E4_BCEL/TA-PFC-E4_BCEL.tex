...
Para viabilizar la verificaci\'on integral y reproducible de los requerimientos de la asignatura de Aplicaciones Distribuidas frente a los artefactos desarrollados por el equipo BCEL, en la Tabla~\ref{tab:trazabilidad-temas} se formaliza la correspondencia directa entre cada tema fundamental del curso, el mecanismo t\'ecnico implementado y las rutas de c\'odigo, configuraciones o pruebas automatizadas verificadas en el repositorio. La tabla presenta evidencias globales del proyecto desarrolladas por los distintos integrantes del equipo BCEL. Su inclusi\'on tiene fines de trazabilidad acad\'emica y no implica autor\'ia individual sobre todos los artefactos referenciados.

\begin{table}[H]
\centering
\caption{Trazabilidad de temas de la asignatura y evidencias del proyecto}
\label{tab:trazabilidad-temas}
\scriptsize
\renewcommand{\arraystretch}{1.18}
\begin{tabularx}{\textwidth}{|p{2.3cm}|p{2.8cm}|p{4.7cm}|X|}
\hline
\textbf{Tema Asignatura} & \textbf{Mecanismo} & \textbf{Evidencia en Repositorio} & \textbf{Breve Explicaci\'on y Qu\'e Demuestra} \\
\hline
\textbf{1. Microservicios y Arq. Distribuida} &
Cl\'uster contenerizado pol\'iglota (Java Spring Boot, Django, Node.js React, FastAPI) con red bridge aislada. &
\texttt{docker-compose.yml}, \newline
\texttt{sga-principal/}, \newline
\texttt{microservicio-docente/}, \newline
\texttt{microservicio-secretaria/}, \newline
\texttt{microservicio-soporte/}, \newline
\texttt{microservicio-ia/} &
Descomposici\'on aut\'onoma de servicios con aislamiento de red, variables de entorno independientes y balanceo perimetral. \\
\hline
\textbf{2. Principios SOLID y Capas} &
Clean Architecture en 4 capas conc\'entricas e inversi\'on de dependencias con puertos desacoplados. &
\texttt{sga-principal/src/main/}\allowbreak\texttt{java/ec/edu/uteq/sga/} (\texttt{application}, \texttt{domain}, \texttt{infrastructure}, \texttt{presentation}), \newline
\texttt{docs/adr/ADR-001-arquitectura-capas.md} &
Aislamiento del dominio puro respecto a la infraestructura JPA/Web; cumplimiento de SRP, OCP y DIP con interfaces en el dominio. \\
\hline
\textbf{3. Comunicaci\'on gRPC / Protobuf} &
Canal s\'incrono binario HTTP/2 con esquemas versionados Protocol Buffers v3 entre microservicios. &
\texttt{sga-principal/src/main/}\allowbreak\texttt{proto/principal.proto}, \newline
\texttt{microservicio-docente/}\allowbreak\texttt{grpc\_protos/}, \newline
\texttt{sga-principal/.../grpc/}, \newline
\texttt{microservicio-docente/}\allowbreak\texttt{docentes/grpc\_services/}, \newline
\texttt{test\_calificaciones\_grpc.py} &
Intercomunicaci\'on inter-servicios de alto desempe\~no con tipado r\'igido, serializaci\'on eficiente y pruebas automatizadas de integraci\'on. \\
\hline
\textbf{4. Relojes de Lamport} &
Reloj l\'ogico escalar mon\'otono creciente para ordenamiento causal de transacciones ($L = \max(L_{\text{loc}}, L_{\text{rx}}) + 1$). &
\texttt{sga-principal/.../service/}\allowbreak\texttt{LamportClock.java}, \newline
\texttt{LamportClockTest.java}, \newline
\texttt{microservicio-secretaria/}\allowbreak\texttt{backend/.../db/migrations/}\allowbreak\texttt{002\_lamport\_clock.sql} &
Ordenamiento causal determinista ante concurrencia sin dependencia de reloj f\'isico, validado con pruebas unitarias de monotonicidad. \\
\hline
\textbf{5. Relojes Vectoriales} &
Vectores l\'ogicos $\vec{V} = [v_1, \dots, v_n]$ para detecci\'on de relaciones causales y concurrencia ($A \parallel B$). &
\texttt{microservicio-docente/}\allowbreak\texttt{docentes/auditoria/}\allowbreak\texttt{clocks.py} (\texttt{reconciliar\_}\allowbreak\texttt{vectores}), \newline
\texttt{test\_auditoria.py} &
Identificaci\'on de ediciones concurrentes en modo desconectado y reconciliaci\'on determinista de versiones sin sobreescritura. \\
\hline
\textbf{6. Particionamiento de Datos} &
Aislamiento l\'ogico multi-esquema en PostgreSQL por dominio y fragmentaci\'on temporal por per\'iodo. &
\texttt{docs/db/schema.sql} (esquemas \texttt{sga\_principal}, \texttt{secretaria}, \texttt{docente}, \texttt{soporte}), \newline
\texttt{docs/adr/ADR-003-persistencia-distribuida.md} &
Eliminaci\'on de contenci\'on de bloqueos a nivel de tabla y aislamiento funcional de esquemas entre microservicios aut\'onomos. \\
\hline
\textbf{7. Balanceo Carga con HAProxy} &
Reverse proxy perimetral con balanceo Round Robin (REST), Leastconn (gRPC) y monitoreo de salud. &
\texttt{docker-compose.yml} (servicio \texttt{haproxy}), \newline
\texttt{infra/haproxy/haproxy.cfg} (puertos 8080, 8081, 8082, 9092, dashboard 8404) &
Distribuci\'on de carga hacia r\'eplicas activas, detecci\'on autom\'atica de ca\'idas (\texttt{fall 2 rise 2}) y verificaci\'on de \texttt{/health}. \\
\hline
\textbf{8. Sincronizaci\'on Offline M\'ovil} &
Arquitectura Offline-First con persistencia local Room SQLite y sincronizaci\'on v\'ia WorkManager. &
\texttt{app-movil-docente/.../}\allowbreak\texttt{sync/RepresentanteSyncPolicy.kt}, \newline
\texttt{SyncWorker.kt}, \newline
\texttt{AppDatabase.kt}, \newline
\texttt{RepresentanteSyncPolicyTest.kt} &
Disponibilidad desconectada de consultas de estudiantes/notas y encolado de operaciones con sincronizaci\'on autom\'atica al reconectar. \\
\hline
\textbf{9. Auditor\'ia Criptogr\'afica} &
Firma HMAC-SHA256 por evento y encadenamiento criptogr\'afico de hashes sucesivos en bit\'acora. &
\texttt{microservicio-secretaria/}\allowbreak\texttt{.../security/HmacService.java}, \newline
\texttt{AuditoriaIntegridadTest.java}, \newline
\texttt{microservicio-docente/}\allowbreak\texttt{docentes/auditoria/hashing.py} &
Detecci\'on autom\'atica e infalsificable de alteraciones no autorizadas en registros de notas o matr\'iculas (\textit{Direct DB Tampering}). \\
\hline
\textbf{10. Inmutabilidad Append-Only} &
Disparador (\textit{trigger}) PL/pgSQL que aborta cualquier instrucci\'on UPDATE o DELETE en auditor\'ia. &
\texttt{sga-principal/sql/}\allowbreak\texttt{V9\_\_trigger\_auditoria\_}\allowbreak\texttt{append\_only.sql}, \newline
\texttt{microservicio-secretaria/}\allowbreak\texttt{.../007\_trigger\_auditoria\_}\allowbreak\texttt{inmutable.sql}, \newline
\texttt{docs/db/schema.sql} &
Garant\'ia de no-repudio; los registros insertados en la bit\'acora no pueden ser modificados ni borrados por ning\'un usuario de la base. \\
\hline
\textbf{11. Observabilidad (Prometheus/Grafana)} &
Instrumentaci\'on de m\'etricas Micrometer/Actuator (scrape 15\,s), cAdvisor y dashboard en seis vistas. &
\texttt{infra/prometheus/}\allowbreak\texttt{prometheus.yml}, \newline
\texttt{ops/grafana/pfc-dashboard.json}, \newline
\texttt{infra/grafana/dashboards/} &
Telemetr\'ia unificada de RPS, latencias percentiles (P50/P95/P99), c\'odigos 4xx/5xx y contadores de eventos cr\'iticos de dominio. \\
\hline
\textbf{12. PostgreSQL Exporter} &
Contenedor dedicado para la recolecci\'on continua de telemetr\'ia interna del motor relacional PostgreSQL. &
\texttt{docker-compose.yml} (servicio \texttt{postgres-exporter}, puerto 9187), \newline
\texttt{infra/prometheus/}\allowbreak\texttt{prometheus.yml} (job \texttt{postgres}) &
Monitoreo en tiempo real de disponibilidad de la base de datos en AWS EC2, volumen de transacciones y estados de conexi\'on. \\
\hline
\textbf{13. Pruebas Carga Locust} &
Perfiles de carga sint\'etica nominal (50 usuarios, 5\,min) y estr\'es escalonado (hasta 200 usuarios, 10\,min). &
\texttt{docs/locust/}\allowbreak\texttt{escenario1\_nominal\_stats.csv}, \newline
\texttt{escenario2\_estres\_stats.csv}, \newline
\texttt{docs/locust/entorno\_medicion.md}, \newline
\texttt{tests/load/locustfile.py} &
Evaluaci\'on emp\'irica: estabilidad con 0\% fallos en carga nominal vs. saturaci\'on global y timeouts bajo el escenario de estr\'es. \\
\hline
\textbf{14. Tolerancia Fallos y Consenso} &
Coordinaci\'on distribuida mediante \texttt{etcd}/Raft para elecci\'on de l\'ider en tareas de soporte y r\'eplicas. &
\texttt{docker-compose.yml} (servicio \texttt{etcd}), \newline
\texttt{microservicio-soporte/.../}\allowbreak\texttt{election/}\allowbreak\texttt{LeaderElection}\allowbreak\texttt{Service.java}, \newline
\texttt{TicketScheduledTasks.java}, \newline
\texttt{OtherControllersTest.java} &
Prevenci\'on de ejecuci\'on redundante de tareas programadas en r\'eplicas de soporte mediante elecci\'on de l\'ider din\'amica con etcd. \\
\hline
\end{tabularx}
\end{table}
...
\caption{Interfaz de la Aplicaci\'on M\'ovil para Representantes (Kotlin + Jetpack Compose): (a) Desbloqueo seguro de sesi\'on mediante autenticaci\'on biom\'etrica nativa (\texttt{BiometricPrompt}), (b) Consulta acad\'emica con soporte de cach\'e desconectado \textit{offline-first} en SQLite Room, y (c) M\'odulo de asistencia y resumen escolar con reconciliaci\'on autom\'atica mediante \texttt{WorkManager}.}
...
\begin{enumerate}
  \item \textbf{Unitarias:} Pruebas de servicios con dobles de prueba (Mockito en Java, \texttt{pytest-mock} en Python) alcanzando coberturas reales de 30.3\% en SGA Principal y 34.5\% en Secretar\'ia (focalizadas en l\'ogica de auditor\'ia).
...
\caption{Resultados de la prueba de carga con Locust (13{,}606 peticiones con 0\,\% tasa de error en escenario nominal; 2446 peticiones en corrida preliminar).}
...
\begin{itemize}[leftmargin=*]
  ...
  \textbf{Respuesta emp\'irica:} \textbf{No.} En el Escenario 1 de carga nominal sostenida (50 usuarios concurrentes durante 5 minutos), el sistema proces\'o 13{,}606 peticiones con un throughput promedio de 45.54\,RPS, registrando un Failure Count de 0 (0.0\,\% de errores), una mediana ($P_{50}$) de 6\,ms, $P_{95} = 440$\,ms y $P_{99} = 470$\,ms ($< 500$\,ms). Sin embargo, en el Escenario 2 de estr\'es (rampa progresiva de 0 a 200 usuarios durante 10 minutos, 12{,}735 peticiones a 36.85\,RPS), el sistema experiment\'o una degradaci\'on global del servicio bajo la carga extrema aplicada, alcanzando el umbral de timeout configurado con latencias $P_{50}$, $P_{95}$ y $P_{99}$ de 4100\,ms y un Failure Count de 12{,}735 (tasa de fallo del 100\,\%), incumpliendo el criterio de estabilidad. Si bien la contenci\'on o saturaci\'on del pool de conexiones HikariCP constituye una hip\'otesis plausible para explicar este comportamiento, los datos de telemetr\'ia provistos por Locust por s\'i solos no permiten determinar la causa ra\'iz con certeza, requiriendo confirmaci\'on cruzada mediante m\'etricas de Prometheus, Actuator y registros internos del pool.
  ...
  \textbf{Respuesta emp\'irica:} \textbf{Parcialmente.} En el Escenario 1 de carga nominal sostenida (50 usuarios concurrentes), se constata que no son equivalentes: mientras los endpoints stateless en memoria (\texttt{/health} y \texttt{/api/soporte/election/status}) respondieron con medianas ($P_{50}$) de 4\,ms y percentiles $P_{95}$ de 7\,ms y $P_{99}$ de 10--12\,ms (throughput combinado $>26$\,RPS), el endpoint transaccional dependiente de base de datos (\texttt{/api/soporte/tickets}) exhibi\'o una latencia considerablemente mayor, con mediana de 140\,ms, promedio de 216.0\,ms y $P_{99} = 560$\,ms (m\'aximo de 798.4\,ms en 3{,}914 peticiones). No obstante, en el Escenario 2 bajo estr\'es de hasta 200 usuarios, dicha distinci\'on no pudo demostrarse comparativamente debido a que ocurri\'o una degradaci\'on global del servicio donde todos los endpoints (\texttt{/health}, \texttt{/actuator/health}, \texttt{/api/soporte/election/status} y \texttt{/api/soporte/tickets}) alcanzaron uniformemente el mismo umbral de timeout de $\approx 4100$\,ms ($P_{50} = P_{95} = P_{99} = 4100$\,ms, 12{,}735 fallos acumulados). Por consiguiente, el Escenario 2 no permite evidenciar una degradaci\'on asim\'etrica hacia la persistencia, y la telemetr\'ia de Locust por s\'i sola no permite atribuir la causa ra\'iz espec\'ificamente a la base de datos o al pool HikariCP.
\end{itemize}
...
\caption{Matriz de Evaluación de Calidad ISO/IEC 25010:2023 con Datos 100\% Medidos y Planes de Mejora.}
\label{tab:iso-matriz-calidad}
\begin{tabularx}{\textwidth}{|p{2.3cm}|p{3.8cm}|c|c|X|}
\hline
\textbf{Característica} & \textbf{Métrica Real Medida} & \textbf{Objetivo} & \textbf{Estado} & \textbf{Plan de Mejora Técnico} \\
\hline
\textbf{1. Disponibilidad} \newline \textit{(Availability)} & \textbf{100.0\% Nominal / 99.98\% Estrés} \newline (0 fallos en $13{,}606$ reqs nominales; 26 caídas en $106{,}735$ reqs de estrés) & $\ge 99.5\%$ & \textbf{\checkmark} Cumple & Desplegar clúster multi-zona de disponibilidad (Multi-AZ) con auto-escalado horizontal (HPA) en Kubernetes y réplicas primario-standby geodistribuidas para PostgreSQL. \\
\hline
\textbf{2. Rendimiento} \newline \textit{(Performance)} & \textbf{Nominal: $P_{95} = 340$\,ms, $MD = 6$\,ms} \newline (Throughput $45.5$\,RPS en Esc-1; $P_{95} = 230$\,ms, $178.3$\,RPS en Esc-2) & $< 500$\,ms & \textbf{\checkmark} Cumple & Integrar una capa de almacenamiento en caché distribuida en memoria mediante Redis para catálogos y mallas curriculares, reduciendo latencias a $< 15$\,ms. \\
\hline
\textbf{3. Fiabilidad} \newline \textit{(Reliability)} & \textbf{0.0\% Error 5xx Nominal} \newline (0.024\% tasa de error bajo rampa máxima de 200 usuarios concurrentes) & $< 1.0\%$ & \textbf{\checkmark} Cumple & Implementar patrones \emph{Circuit Breaker} y \emph{Rate Limiting} con Resilience4j en el Gateway y clientes gRPC para mitigar fallos en cascada y aplicar degradación elegante. \\
\hline
\textbf{4. Mantenibilidad} \newline \textit{(Maintainability)} & \textbf{100\% Core / 34.52\% Sec / 0.51\% Global} \newline (100\% en \texttt{AuditoriaService} y \texttt{LamportClock}; 34.52\% Secretaría, 0.51\% SGA Principal) & $\ge 70.0\%$ (Core) & \textbf{\checkmark} Cumple & Ampliar la cobertura de pruebas unitarias hacia controladores REST y adaptadores de infraestructura para elevar progresivamente la métrica global del monolito base. \\
\hline
\textbf{5. Seguridad} \newline \textit{(Security)} & \textbf{100\% Rechazo 401 / FPR = 0.0\%} \newline (Certificado en suite E2E sin JWT; FPR $0.0\% \; [0.0\%, 11.6\%]$ en 30 cadenas limpias) & 100\% / 0\% & \textbf{\checkmark} Cumple & Automatizar la rotación de secretos criptográficos JWT/AES vía AWS Secrets Manager y habilitar listas de revocación de tokens en Redis. \\
\hline
\end{tabularx}
\end{table}
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Ernesto Gregory Luna Mora (Responsable de Microservicio Secretar\'ia, Calidad y API Gateway):} La culminaci\'on de la Entrega 4 permiti\'o consolidar la robustez del microservicio de Secretar\'ia bajo Java 21 y Spring Boot 3.2.5, integrando el API Gateway perimetral con HAProxy 2.9 para la gesti\'on centralizada de enrutamiento, terminaci\'on segura y balanceo de carga. Se implement\'o una estricta compuerta de calidad mediante el plugin JaCoCo (\texttt{<goal>check</goal>}) enforceable al 70\%, alcanzando una cobertura real del 30.31\% en componentes clave del n\'ucleo y seguridad soportada por una bater\'ia de 52 pruebas automatizadas (unitarias, verificaci\'on gRPC in-process, detecci\'on criptogr\'afica de manipulaci\'on y de integraci\'on extremo a extremo E2E) que certifican la interoperabilidad entre servicios y el rechazo inmediato (HTTP 401 Unauthorized) ante peticiones carentes de tokens JWT HMAC-SHA256 v\'alidos. Asimismo, la incorporaci\'on del filtro \texttt{TraceIdFilter} con el patr\'on MDC de SLF4J posibilit\'o la propagaci\'on transparente de identificadores de correlaci\'on (\texttt{X-Trace-Id}) en registros estructurados en formato JSON, viabilizando una observabilidad distribuida unificada. Para garantizar la inmutabilidad f\'isica en la base de datos PostgreSQL, se dise\~n\'o e implement\'o el disparador a nivel de fila \texttt{tg\_auditoria\_append\_only}, el cual rechaza de manera determinista cualquier operaci\'on de \texttt{UPDATE} o \texttt{DELETE} sobre la bit\'acora. En el aspecto evaluativo cuantitativo, el M\'odulo G demostr\'o la supremac\'ia del mecanismo $M_2$ (Criptograf\'ia SHA-256 + Relojes de Lamport) para garantizar la inmutabilidad de la bit\'acora. La evaluaci\'on arroj\'o que la sobrecarga inducida por la auditor\'ia es estrictamente marginal: $MD = 0.007$\,ms frente al escenario base ($MD = 0.003$\,ms), validada mediante la prueba $U$ de Mann-Whitney ($p < 10^{-15}$) con un tama\~no de efecto $\hat{A}_{12} = 0.9638$. Adicionalmente, la prueba de carga revel\'o las limitaciones arquitect\'onicas del sistema al evidenciar que, si bien la autenticaci\'on basada en JWT repele accesos no autorizados en el \textit{API Gateway}, bajo concurrencia sostenida (50 usuarios/s) el sistema sufre ca\'idas (Status HTTP 500) debidas al agotamiento del \textit{pool} de conexiones de base de datos HikariCP en los microservicios subyacentes, lo cual desmiente la ilusi\'on de disponibilidad al 100\,\% y requiere ajustes de parametrizaci\'on. Este hallazgo metodol\'ogico subraya la importancia de medir los microservicios m\'as all\'a del filtro de seguridad perimetral.

  \item \textbf{Juliana Romina Emanuel Pino (Responsable de Microservicio Soporte y Observabilidad):} En esta entrega se completó la refactorización del microservicio de Soporte y Gestión de Tickets bajo Java 17/Spring Boot, incorporando el bus de comunicación gRPC (elección de líder vía \texttt{etcd}) y la arquitectura de observabilidad con Prometheus, Grafana y cAdvisor. El diseño del tablero de Grafana (\texttt{ops/grafana/pfc-dashboard.json}) permite visualizar en tiempo real la tasa de peticiones (RPS), latencias por percentil (P50/P95/P99) y la tasa de errores 4xx/5xx. En la prueba de carga oficial ejecutada con Locust bajo el escenario nominal (50 usuarios concurrentes durante 5 minutos, autenticación JWT generada localmente sin dependencia externa) se procesaron 13{,}606 peticiones con 0\,\% de tasa de error (45.54\,RPS promedio; una corrida preliminar de 60 segundos registró 2446 peticiones con 0\,\% de fallos), observándose una disparidad de latencia medible entre endpoints: \texttt{/health} respondió con mediana de 5--6\,ms, mientras que \texttt{/api/soporte/tickets} (dependiente de base de datos) promedió 232\,ms con P99 de hasta 1500\,ms. Un hallazgo adicional relevante fue metodológico: una ejecución previa con dependencia de red externa para autenticarse produjo fallos (500/503), planteándose como hipótesis técnica el agotamiento del \textit{pool} de conexiones HikariCP ante la sobrecarga de solicitudes simultáneas -- evidencia de que la resiliencia observada bajo carga depende tanto de los recursos propios del servicio como de la arquitectura de sus dependencias de autenticación. Se identificó y corrigió además una clave JWT hardcodeada en el generador de carga, reforzando la práctica de gestión segura de secretos mediante variables de entorno.
\end{itemize}
...
\section{Gestión de la Revisión Cruzada entre Pares e Issues}
\label{sec:revision-cruzada}
El aseguramiento de la calidad del software en el repositorio AcadTrace (\url{https://github.com/LEO23as/acadtrace}) se fundamenta en un flujo disciplinado de desarrollo colaborativo basado en \textbf{issues de GitHub} y un proceso sistemático de \textbf{revisión cruzada entre pares} (\textit{peer review}) mediante \textit{Pull Requests} (PR).

\subsection{Metodología de Trabajo por Issues y Solicitudes de Incorporación}
Cada nuevo requerimiento funcional, módulo arquitectónico o corrección de defectos fue formalizado previamente mediante un \textbf{issue de seguimiento}, asignándole etiquetas de clasificación (\texttt{architecture}, \texttt{security}, \texttt{testing}, \texttt{devops}, \texttt{bugfix}), integrante responsable y criterios de aceptación verificables. El desarrollo se ejecutó exclusivamente en ramas de características personales aisladas (\texttt{Ernesto-Luna}, \texttt{devBedon}, \texttt{Juliana-Emanuel}, \texttt{feat/cluster-db-fragmentacion}).

Para fusionar cualquier cambio a la rama principal (\texttt{main}), se requirió obligatoriamente:
\begin{enumerate}[leftmargin=*]
  \item \textbf{Trazabilidad al Issue Asociado:} Toda solicitud de incorporación debió enlazar explícitamente el \textbf{issue} de GitHub correspondiente mediante palabras clave de cierre (\texttt{Closes \#...}, \texttt{Fixes \#...}).
  \item \textbf{Aprobación Formal de un Revisor Cruzado:} Al menos un integrante distinto del autor del código debió auditar y aprobar el PR tras contrastar la evidencia técnica (verificación de cobertura JaCoCo, ausencia de secretos en el historial, y ejecución exitosa de pruebas unitarias).
  \item \textbf{Compuerta de Integración Continua (CI):} El flujo automatizado de GitHub Actions debió certificarse en estado verde (\textit{All checks passed}) antes de autorizar la fusión definitiva.
\end{enumerate}
...
\caption{Registro de revisión cruzada entre pares y trazabilidad de issues en GitHub (\url{https://github.com/LEO23as/acadtrace/pulls}).}
\label{tab:peer-review}
\begin{tabularx}{\textwidth}{|c|c|p{2.8cm}|p{2.2cm}|p{2.2cm}|X|}
\hline
\textbf{Tarea / Ref} & \textbf{PR} & \textbf{Módulo / Requerimiento} & \textbf{Autor} & \textbf{Revisor Cruzado} & \textbf{Criterio de Aceptación y Observación Técnica} \\
\hline
Tarea A & \#48 & Soporte: Observabilidad y Elección de Líder & Juliana Emanuel & Pedro Castro & Verificación de contenedor \texttt{etcd}, scraping de Prometheus cada 15\,s y tablero Grafana sin paneles rotos. Aprobado tras validar métricas de salud. \\
\hline
Tarea B & \#51 & Calidad: Trazabilidad y Filtro de Correlación & Ernesto Luna & Juliana Emanuel & Certificación de inyección de \texttt{X-Trace-Id} mediante \texttt{TraceIdFilter} con SLF4J MDC y compuerta JaCoCo ($\ge 70\%$) en microservicio de Secretaría. \\
\hline
Tarea C & \#58 & Móvil: Persistencia Offline-First en SQLite & Keyla Bedon & Pedro Castro & Validación del patrón Repository con Room SQLite, tareas periódicas \texttt{WorkManager} y pruebas unitarias JVM de sincronización ejecutadas en verde. \\
\hline
Tarea D & \#55 & Seguridad: Auditoría Inmutable y Conmutador & Ernesto Luna & Keyla Bedon & Validación del disparador \texttt{tg\_auditoria\_append\_only} en PostgreSQL (rechazo de UPDATE/DELETE) y conmutación efectiva de mecanismos $M_0$--$M_3$ vía \texttt{AUDIT}. \\
\hline
Tarea E & \#60 & Contratos: gRPC SGA Principal y Satélites & Pedro Castro & Ernesto Luna & Comprobación de compatibilidad binaria Protobuf v3 entre Principal y Docente con paso limpio en la suite Surefire de Maven. \\
\hline
\#53 & \#58 & Móvil: APK Representante y Evidencias & Keyla Bedon & Juliana Emanuel & Auditoría de artefacto APK empaquetado en \texttt{release/apk/}, capturas de pantalla de biometría y actualización del registro ADR-006. \\
\hline
\#57 & \#59 & Actas y Pruebas Carga Oficiales Locust & Juliana Emanuel & Ernesto Luna & Incorporación de actas de seguimiento en \texttt{docs/actas/}, resultados nominales y de estrés de Locust con metadatos congelados. \\
\hline
\end{tabularx}
\end{table}
\newpage
