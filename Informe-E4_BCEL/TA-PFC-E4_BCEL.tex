...
En cumplimiento de la transparencia académica exigida por la cátedra, se declara el uso de asistentes de inteligencia artificial generativa (Claude, Antigravity/Gemini) durante el desarrollo de la Entrega 4, con el siguiente alcance por integrante:
...
\textit{[NOTA PARA EL EQUIPO: cada integrante debe completar aquí su propia declaración de uso de IA, con el mismo nivel de detalle -- qué herramienta, para qué tarea, y qué se verificó manualmente antes de aceptar el resultado. No dejar esta sección incompleta.]}

\section{Revisión Cruzada entre Pares}
\textit{[NOTA PARA EL EQUIPO: esta tabla debe completarse con datos REALES de GitHub (Pull Requests revisados, comentarios de revisión, aprobaciones) -- NO inventar filas. Ejemplo de estructura esperada:]}

\begin{table}[H]
\centering
\begin{tabular}{p{2.5cm}p{2.5cm}p{2cm}p{5cm}}
\toprule
\textbf{PR revisado} & \textbf{Autor} & \textbf{Revisor} & \textbf{Observación principal} \\
\midrule
\#XX & Nombre & Nombre & [completar con el comentario real de revisión] \\
\bottomrule
\end{tabular}
\caption{Registro de revisión cruzada entre integrantes del equipo (completar con PRs reales de \url{https://github.com/LEO23as/acadtrace/pulls?q=is:pr})}
\end{table}
\newpage
