...
Aislamiento multi-esquema en PostgreSQL por dominio y fragmentaci\'on por per\'iodo. &
\texttt{docs/db/schema.sql} &
12--16 \newline
85--110 &
P. Castro, \newline K. Bed\'on &
Eliminaci\'on de contenci\'on de bloqueos de tabla entre servicios aut\'onomos (\texttt{sga\_principal}, \texttt{docente}, etc.). \\
\hline
...
\texttt{microservicio-secretaria/backend/}\allowbreak\texttt{src/main/resources/db/migrations/}\allowbreak\texttt{007\_trigger\_auditoria\_inmutable.sql} \newline
\texttt{docs/db/schema.sql} &
11--26 \newline
156--170 &
E. Luna, \newline P. Castro &
...
\begin{enumerate}
  \item \textbf{Unitarias:} Evidencia hist\'orica conservada de pruebas de servicios con dobles de prueba (Mockito en Java, \texttt{pytest-mock} en Python): 30.31\% LINE de SGA Principal y 34.53\% LINE global de Secretar\'ia seg\'un el contador XML de JaCoCo. No fueron regeneradas contra el \textit{HEAD} actual y no constituyen cobertura actual.
...
\caption{Captura histórica/complementaria de Grafana. No acredita el agregado del CSV oficial nominal de Soporte.}
...
\hline
2 & \texttt{test-backend} & Tests unitarios Java (Surefire/JaCoCo, umbral m\'inimo de 70\% LINE) y Python (\texttt{pytest-cov}); el umbral es criterio de calidad, no resultado medido. \\
\hline
...
La única corrida OFICIAL NOMINAL de Soporte es \path{microservicio-soporte/locust_esc1_stats.csv}, acompañada por \path{microservicio-soporte/locust_esc1_stats_history.csv}, \path{microservicio-soporte/locust_esc1_failures.csv} y \path{microservicio-soporte/locust_esc1_exceptions.csv}. Sus métricas proceden de \texttt{Aggregated}: 12{,}994 peticiones, 0 fallos, 43.537580\,req/s, promedio 109.113664\,ms, $P_{50}=6$\,ms, $P_{95}=440$\,ms, $P_{99}=850$\,ms y máximo 2037.104700\,ms; no hay HTTP 401 ni HTTP 500 registrados. Es una prueba de carga reproducible ejecutada en entorno local/contenedorizado, no tráfico real de producción.

El inicio registrado es 2026-09-11 03:59:05 UTC; el historial abarca 299 segundos entre muestras. El perfil configura 50 usuarios virtuales, 5 usuarios/s, 5 minutos y \url{http://localhost:8083}. Commit de conservación del resultado: \texttt{956cafcb}, con normalización posterior en \texttt{c5c0e6f5}. Commit del código ejecutado: \textbf{No disponible en la evidencia conservada}.

La tabla completa de estados, fechas, commits de conservación, métricas y motivos está en \path{experimentos/resultados/corridas-e5.md}. B (12{,}236 peticiones), C (13{,}606) y F (2{,}419) son preliminares/históricas; D y E son fallidas. \textbf{Corrida oficial de estrés: NO DISPONIBLE --- las evidencias conservadas no satisfacen el criterio}. La numeración histórica de estrés difiere entre protocolo (escenario 3) y \path{docs/locust/escenario2_estres_stats.csv}; no son nuevos resultados oficiales.

La captura \path{evidencias/Juliana_Emanuel/backend/pruebas-carga/image.png} es histórica/complementaria: muestra 13{,}031 peticiones en terminal; el CSV oficial conserva 12{,}994 y siempre prevalece. La causa de la diferencia no está demostrada. Falta una captura manual de la ruta del CSV y su fila \texttt{Aggregated}, con Request Count 12994, Failure Count 0 y métricas visibles. Las imágenes de Grafana son complementarias, sin vinculación exacta acreditada con ese agregado.

Como antecedentes, el informe anterior citaba 12{,}265 peticiones (no coincide con B), una calibración de 212 sin CSV localizado, y el protocolo 57.4\,RPS y $P_{95}=285$\,ms sin asociación demostrada a un CSV de este inventario. Estas cifras no son oficiales. \textbf{E5: PARCIAL}; se conservan CSV, capturas, certificados y versiones históricas en Git.
...
\begin{itemize}[leftmargin=*]
  ...
  \textbf{Respuesta empírica:} \textbf{No se satisface el umbral propuesto.} La corrida OFICIAL NOMINAL de Soporte conserva 12{,}994 peticiones, 0 fallos, 43.537580\,req/s, promedio 109.113664\,ms, $P_{50}=6$\,ms, $P_{95}=440$\,ms, $P_{99}=850$\,ms y máximo 2037.104700\,ms. El $P_{99}$ ya supera 500\,ms en nominal. El perfil configurado es de 50 usuarios virtuales, 5 usuarios/s y 5 minutos contra localhost:8083. No hay corrida oficial de estrés válida: el conjunto D histórico tiene 106{,}735 peticiones y 26 fallos (15 HTTP 500 y 11 HTTP 503). La versión histórica de estrés en \texttt{683cc17f} conserva 12{,}735 peticiones fallidas con código 0 y percentiles de 4100\,ms; no debe confundirse con D ni atribuirse a HTTP 500. Locust por sí solo no demuestra la causa raíz ni el estado del pool HikariCP.
...
\hline
\textbf{1. Disponibilidad} \newline \textit{(Availability)} & 0 fallos en 12{,}994 peticiones nominales (100\% de éxito en la muestra local); sin medición oficial válida de estrés & $\ge 99.5\%$ & Parcial: no acredita disponibilidad de producción & Medir disponibilidad temporal y repetir estrés con evidencia válida. \\

\textbf{1. Disponibilidad} \newline \textit{(Availability)} & \textbf{100.0\% Nominal / 0.0\% Estrés (Esc-2)} \newline (Nominal Esc-1: 0 fallos en $12{,}265$ reqs; Estrés Esc-2: degradación global y 100\% fallos por timeout de 4100\,ms bajo 200 usuarios con $12{,}735$ peticiones no atendidas) & $\ge 99.5\%$ \newline (Nominal) & Parcial (Cumple en nominal) & Desplegar clúster multi-zona de disponibilidad (Multi-AZ) con auto-escalado horizontal (HPA) en Kubernetes y réplicas primario-standby geodistribuidas para PostgreSQL para superar el cuello de botella monoinstancia. \\
\hline
\textbf{2. Rendimiento} \newline \textit{(Performance)} & Nominal oficial: $P_{95}=440$\,ms, $P_{50}=6$\,ms, $P_{99}=850$\,ms; 43.537580\,req/s & $P_{95}<500$\,ms & Cumple en agregado nominal local & Evaluar por endpoint: tickets presenta $P_{95}=650$\,ms; repetir estrés. \\
\hline
\textbf{3. Fiabilidad} \newline \textit{(Reliability)} & 0 fallos nominales; D FALLIDA/HISTÓRICA conserva 26 fallos en 106{,}735 peticiones & $<1.0\%$ & Nominal observado; E5 estrés pendiente & Obtener una corrida de estrés autenticada con cero fallos para E5. \\

\textbf{3. Fiabilidad} \newline \textit{(Reliability)} & \textbf{0.0\% Fallos Nominal / 100\% en Estrés} \newline (0 fallos 5xx en Esc-1 de 50 usuarios; en estrés extremo a 200 usuarios el 100\% de peticiones agotó el timeout de 4100\,ms por saturación y contención de recursos) & $< 1.0\%$ \newline (Nominal) & Parcial (Cumple en nominal) & Implementar patrones \emph{Circuit Breaker} y \emph{Rate Limiting} con Resilience4j en el Gateway y clientes gRPC para mitigar fallos en cascada y aplicar degradación elegante ante sobrecargas severas. \\
\hline
\textbf{4. Mantenibilidad} \newline \textit{(Maintainability)} & \textbf{Histórico: 30.31\% LINE SGA Principal / 34.53\% LINE global Secretaría} \newline (100\% histórico en lógica de negocio \texttt{AuditoriaService} y \texttt{LamportClock}; no regenerado contra \textit{HEAD}) & Umbral mínimo: 70\% LINE JaCoCo & Histórico/no actual & Ampliar progresivamente la suite de pruebas unitarias hacia controladores REST y adaptadores de infraestructura para alcanzar la meta global del 70\%. \\
\hline
...
\section{Gestión de Integración de Código, Compuertas de Calidad CI/CD y Trazabilidad de Issues}
\label{sec:revision-cruzada}
El aseguramiento de la calidad del software en el repositorio AcadTrace (\url{https://github.com/LEO23as/acadtrace}) se fundamentó en un flujo estructurado de desarrollo colaborativo apoyado en el seguimiento mediante \textbf{issues de GitHub}, la validación automatizada mediante compuertas de \textbf{Integración Continua (CI/CD)} y la coordinación técnica entre integrantes a través de \textit{Pull Requests} (PR).

\subsection{Metodología de Trabajo por Issues y Solicitudes de Incorporación}
Cada nuevo requerimiento funcional, módulo arquitectónico o corrección de defectos fue formalizado previamente mediante un \textbf{issue de seguimiento}, asignándole etiquetas de clasificación (\texttt{architecture}, \texttt{security}, \texttt{testing}, \texttt{devops}, \texttt{bugfix}), integrante responsable y criterios de aceptación técnicos. El desarrollo se ejecutó en ramas de características personales aisladas (\texttt{Ernesto-Luna}, \texttt{devBedon}, \texttt{Juliana-Emanuel}, \texttt{feat/cluster-db-fragmentacion}).

Para la incorporación y fusión de cambios a la rama principal (\texttt{main}), el flujo de trabajo aplicó los siguientes controles técnicos y compuertas de aseguramiento:
\begin{enumerate}[leftmargin=*]
  \item \textbf{Trazabilidad al Issue Asociado:} Toda solicitud de incorporación debió vincular explícitamente el \textbf{issue} de GitHub correspondiente mediante directivas de cierre (\texttt{Closes \#...}, \texttt{Fixes \#...}) para garantizar la justificación y auditoría de cada cambio.
  \item \textbf{Compuertas Automatizadas de Integración Continua (CI/CD):} Como condición indispensable, los flujos automatizados de GitHub Actions (compilación de código, análisis estático, cobertura JaCoCo, y suites de pruebas unitarias e integración en contenedores) debieron certificarse en estado verde (\textit{All checks passed}) antes de autorizar la fusión definitiva.
  \item \textbf{Validación Colaborativa y Coordinación Técnica:} La compatibilidad de dependencias entre subsistemas (contratos gRPC Protobuf, esquemas de bases de datos y enrutamiento del API Gateway) fue validada y coordinada entre los integrantes responsables, asegurando la resolución colaborativa de conflictos y la estabilidad antes del cierre e integración del PR.
\end{enumerate}
...
\caption{Registro de trazabilidad de Pull Requests, validación colaborativa y seguimiento de issues en GitHub (\url{https://github.com/LEO23as/acadtrace/pulls}).}
\label{tab:peer-review}
\begin{tabularx}{\textwidth}{|c|c|p{2.8cm}|p{2.2cm}|p{2.2cm}|X|}
\hline
\textbf{Tarea / Ref} & \textbf{PR} & \textbf{Módulo / Requerimiento} & \textbf{Autor} & \textbf{Coordinación y Validación} & \textbf{Criterio Técnico y Validación Automatizada en CI} \\
\hline
Tarea A & \#48 & Soporte: Observabilidad y Elección de Líder & Juliana Emanuel & Pedro Castro & Verificación de contenedor \texttt{etcd}, scraping de Prometheus cada 15\,s y tablero Grafana. Validación técnica exitosa con métricas de salud en CI. \\
\hline
Tarea B & \#51 & Calidad: Trazabilidad y Filtro de Correlación & Ernesto Luna & Juliana Emanuel & Certificación de inyección de \texttt{X-Trace-Id} mediante \texttt{TraceIdFilter} con SLF4J MDC y compuerta JaCoCo superada en el microservicio de Secretaría. \\
\hline
Tarea C & \#58 & Móvil: Persistencia Offline-First en SQLite & Keyla Bedon & Pedro Castro & Validación del patrón Repository con Room SQLite, tareas periódicas \texttt{WorkManager} y suite de pruebas JVM ejecutadas en verde en GitHub Actions. \\
\hline
...
\hline
Tarea E & \#60 & Contratos: gRPC SGA Principal y Satélites & Pedro Castro & Ernesto Luna & Comprobación de compatibilidad binaria Protobuf v3 entre Principal y Docente con paso limpio en la suite Surefire de Maven en CI. \\
\hline
Tarea F & \#58 & Móvil: APK Representante y Evidencias & Keyla Bedon & Juliana Emanuel & Auditoría de artefacto APK empaquetado en \texttt{release/apk/}, capturas de pantalla de biometría y actualización del registro ADR-006. \\
\hline
Tarea G & \#59 & Actas y Pruebas Carga Oficiales Locust & Juliana Emanuel & Ernesto Luna & Incorporación de actas de seguimiento en \texttt{docs/actas/}, resultados nominales y de estrés de Locust con metadatos congelados. \\
\hline
\end{tabularx}
\end{table}
\newpage
