...
\input{cifras_carga_generadas.tex}
...
\hline
\textbf{1. Disponibilidad} \newline \textit{(Availability)} & \CargaFallos{} fallos en \CargaPeticiones{} peticiones nominales, con un máximo observado de \CargaUsuariosMaximos{} usuarios (100\% de éxito en la muestra local); sin medición oficial válida de estrés & $\ge 99.5\%$ & Parcial: no acredita disponibilidad de producción & Medir disponibilidad temporal y repetir estrés con evidencia válida. \\

\hline
\textbf{2. Rendimiento} \newline \textit{(Performance)} & Nominal oficial: $P_{95}=\CargaPnoventaCincoMs$\,ms, $P_{50}=\CargaPcinquentaMs$\,ms, $P_{99}=\CargaPnoventaNueveMs$\,ms; \CargaRPS{}\,req/s & $P_{95}<500$\,ms & Cumple en agregado nominal local & Evaluar por endpoint: tickets presenta $P_{95}=650$\,ms; repetir estrés. \\
\hline
