...
\textbf{3. Fiabilidad} \newline \textit{(Reliability)} & 0.0\% Fallos Nominal / Estrés Pendiente & $< 1.0\%$ \newline (Nominal) & Parcial (Cumple en nominal) & Implementar patrones \emph{Circuit Breaker} y \emph{Rate Limiting} con Resilience4j en el Gateway y clientes gRPC para mitigar fallos en cascada y aplicar degradación elegante ante sobrecargas severas. \\
\hline
\textbf{4. Mantenibilidad} \newline \textit{(Maintainability)} & Cobertura parcial no consolidada en HEAD & Umbral mínimo: 70\% LINE JaCoCo & Histórico/no actual & Ampliar progresivamente la suite de pruebas unitarias hacia controladores REST y adaptadores de infraestructura para alcanzar la meta global del 70\%. \\
\hline
