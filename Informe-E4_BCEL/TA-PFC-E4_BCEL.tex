...
\begin{figure}[H]
    ...
    \caption{Aplicaci\'on m\'ovil del representante con la solicitud de acceso mediante huella dactilar visible.}
...
\begin{figure}[H]
    ...
    \caption{Aplicaci\'on m\'ovil del representante en la vista de calificaciones, con el aviso de modo sin conexi\'on visible.}
...
\begin{figure}[H]
    ...
    \caption{Aplicaci\'on m\'ovil del representante en la vista de asistencia, con resumen y registros fechados visibles.}
...
La Tabla~\ref{tab:iso-matriz-calidad} presenta la matriz formal de evaluaci\'on de calidad ISO/IEC 25010 generada de forma reproducible a partir de la evidencia experimental auditada (\texttt{matriz\_iso25010\_generada.tex}). El alcance principal se delimita a la corrida nominal local oficial del Microservicio de Soporte (Conjunto~A, con \CargaPeticiones{} peticiones y \CargaFallos{} fallos observados a \CargaUsuariosMaximos{} usuarios concurrentes). Conforme a este alcance, la latencia agregada $P_{95}=\CargaPnoventaCincoMs$\,ms cumple el umbral ($P_{95}<500$\,ms) exclusivamente en el escenario nominal local medido. Las 12{,}994 peticiones con 0 fallos constituyen evidencia parcial de la ejecuci\'on nominal y no demuestran fiabilidad global ni disponibilidad de producci\'on. La fila de Fiabilidad distingue adem\'as ventanas derivadas hist\'oricas de \texttt{locust\_esc3} con perfil no validado: sus fallos no acreditan una prueba de estr\'es oficial actual. La fila de Seguridad incorpora la tasa de falsos positivos de pruebas sint\'eticas M2 desde un CSV versionado; no mide seguridad en producci\'on ni certifica seguridad global. Adecuaci\'on funcional, mantenibilidad, usabilidad, portabilidad y compatibilidad mantienen el veredicto \textit{No evaluado} dentro de este alcance:
...
La trazabilidad de cambios en el repositorio AcadTrace (\url{https://github.com/gleiston-guerrero/acadtrace}) se documenta mediante \textbf{issues de GitHub}, compuertas de \textbf{Integración Continua (CI/CD)} y \textit{Pull Requests} (PR). Para los seis PR citados en la tabla, la API de GitHub registra el autor y la fusión, pero no registra revisiones formales en el endpoint \texttt{/reviews}.
...
\hline
Tarea F & \#58 & APK de representante y evidencias & Ernesto Luna (\texttt{Ernesto835}) & Incluye el APK en \texttt{release/apk/}, capturas y la actualización de ADR-006. \\
\hline
