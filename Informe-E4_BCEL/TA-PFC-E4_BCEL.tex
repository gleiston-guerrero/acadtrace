...
La Tabla~\ref{tab:iso-matriz-calidad} presenta la matriz formal de evaluaci\'on de calidad ISO/IEC 25010 generada de forma reproducible a partir de la evidencia experimental auditada (\texttt{matriz\_iso25010\_generada.tex}). El alcance se delimita a la corrida nominal local oficial del Microservicio de Soporte (Conjunto~A, con \CargaPeticiones{} peticiones y \CargaFallos{} fallos observados a \CargaUsuariosMaximos{} usuarios concurrentes). Conforme a este alcance, la latencia agregada $P_{95}=\CargaPnoventaCincoMs$\,ms cumple el umbral ($P_{95}<500$\,ms) exclusivamente en el escenario nominal local medido. Las 12{,}994 peticiones con 0 fallos constituyen evidencia parcial de la ejecuci\'on nominal y no demuestran fiabilidad global, disponibilidad de producci\'on ni un comportamiento de estr\'es exitoso. Las caracter\'isticas de adecuaci\'on funcional, mantenibilidad, seguridad, usabilidad, portabilidad y compatibilidad se registran con el veredicto expl\'icito de \textit{No evaluado} al no existir evidencia reproducible auditada dentro de los l\'imites de esta matriz; dicho veredicto no significa que el sistema incumpla, sino que la evidencia disponible no permite emitir ese dictamen:

\begin{table}[H]
\centering
\scriptsize
\renewcommand{\arraystretch}{1.2}
\caption{Matriz de Evaluaci\'on de Calidad ISO/IEC 25010:2023 con Mediciones Parciales y Delimitaci\'on de Alcance.}
\label{tab:iso-matriz-calidad}
\input{matriz_iso25010_generada.tex}
\end{table}
