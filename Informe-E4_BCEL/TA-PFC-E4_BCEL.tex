...
\addlinespace
\textbf{4. Mantenibilidad} \newline \textit{(Maintainability)} & \textbf{Cobertura JaCoCo: 90.7\%} \newline (52 pruebas automatizadas, validaci\'on enforceable al 70\% con \texttt{<goal>check</goal>}) & $\ge 70.0\%$ & \textbf{\checkmark} Cumple & Desacoplar el motor de generaci\'on de PDFs (Apache PDFBox) hacia un servicio documental as\'incrono orientado a colas de eventos (RabbitMQ). \\
\addlinespace
...
En cumplimiento de la transparencia académica exigida por la cátedra, se declara el uso de asistentes de inteligencia artificial generativa (Claude, Antigravity/Gemini, GitHub Copilot) durante el desarrollo de la Entrega 4, con el siguiente alcance por integrante:

\begin{itemize}[leftmargin=*]
  \item \textbf{Pedro Leonardo Castro López:} Asistencia en la estructuración de controladores Spring Boot, mediación de contratos gRPC y sintaxis de flujos en GitHub Actions. La lógica de negocio, patrones GoF y compuertas de calidad fueron implementados y verificados manualmente mediante pruebas unitarias Mockito y JaCoCo.
  \item \textbf{Keyla Betzabe Bedon Viteri:} Asistencia en la estructuración de modelos Django REST y consultas Jetpack Compose/Room SQLite. La lógica de sincronización offline con WorkManager y conmutación de relojes lógicos fue implementada y validada manualmente en emulador Android y dispositivos físicos.
  \item \textbf{Ernesto Gregory Luna Mora:} Asistencia en el diseño de filtros JWT en Spring Security y configuración inicial de HAProxy. La propagación de identificadores de traza MDC y las compuertas JaCoCo al 83.0\% fueron certificadas con ejecuciones de pruebas E2E.
  \item \textbf{Juliana Romina Emanuel Pino:} Uso de Claude y Antigravity para generación asistida de la prueba de carga (\texttt{locustfile.py}), configuración de Prometheus/Grafana/cAdvisor, corrección de una vulnerabilidad de secreto JWT hardcodeado, redacción asistida de secciones del informe individual y verificación bibliográfica. Todo hallazgo técnico documentado (latencias, tasas de error) proviene de ejecuciones reales verificadas manualmente, no de datos generados por el modelo de IA.
\end{itemize}

\section{Revisión Cruzada entre Pares}

\begin{table}[H]
\centering
\begin{tabular}{p{1.5cm}p{3.2cm}p{3.2cm}p{6.5cm}}
\toprule
\textbf{PR} & \textbf{Autor} & \textbf{Revisor} & \textbf{Observación de Revisión} \\
\midrule
\#48 & Juliana Emanuel & Pedro Castro & Aprobación de arquitectura soporte y observabilidad Prometheus/Grafana. \\
\#54 & Keyla Bedon & Pedro Castro & Revisión de app móvil offline y sincronización Room. \\
\#55 & Ernesto Luna & Juliana Emanuel & Validación de compuerta JaCoCo (90.7\%) y Gateway HAProxy. \\
\#56 & Pedro Castro & Ernesto Luna & Integración de contratos gRPC entre Principal y Docente. \\
\bottomrule
\end{tabular}
\caption{Registro de revisión cruzada entre integrantes del equipo verificado contra el historial oficial de Pull Requests en GitHub (\url{https://github.com/LEO23as/acadtrace/pulls}).}
...
\section{Revisión Cruzada entre Pares}
El aseguramiento de calidad del repositorio AcadTrace se fundamenta en un proceso sistem\'atico de revisi\'on entre pares soportado mediante \textit{Pull Requests} (PR) reales en GitHub. La Tabla~\ref{tab:peer-review} resume las revisiones cruzadas ejecutadas durante la Entrega 4:

\begin{table}[H]
\centering
\small
\begin{tabular}{p{2.0cm}p{3.2cm}p{3.2cm}p{6.0cm}}
\toprule
\textbf{PR revisado} & \textbf{Autor} & \textbf{Revisor} & \textbf{Observaci\'on t\'ecnica principal} \\
\midrule
\#48 & Juliana Emanuel & Pedro Castro & Aprobaci\'on de la arquitectura hexagonal de soporte y monitoreo en tiempo real con Prometheus y Grafana. \\
\addlinespace
\#51 & Ernesto Luna & Juliana Emanuel & Verificaci\'on de la compuerta JaCoCo ($\ge 70\%$) y trazabilidad estructurada JSON mediante \texttt{TraceIdFilter}. \\
\addlinespace
\#54 & Keyla Bedon & Pedro Castro & Validaci\'on de persistencia local en Room SQLite, arquitectura \textit{offline-first} y pruebas JVM en app m\'ovil. \\
\addlinespace
\#55 & Ernesto Luna & Keyla Bedon & Conmutador din\'amico de auditor\'ia criptogr\'afica ($M_2$) y disparadores \textit{append-only} en PostgreSQL. \\
\addlinespace
\#56 & Pedro Castro & Ernesto Luna & Certificaci\'on de contratos gRPC entre SGA Principal y sat\'elites con paso exitoso en suite Surefire. \\
\bottomrule
\end{tabular}
\caption{Registro de revisi\'on cruzada entre integrantes del equipo BCEL en GitHub (\url{https://github.com/LEO23as/acadtrace/pulls}).}
