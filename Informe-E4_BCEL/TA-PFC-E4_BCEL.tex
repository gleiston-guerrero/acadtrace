...
La Figura~\ref{fig:grafana-dashboard} conserva la evidencia visual del tablero operativo de Grafana utilizado para observar el comportamiento de los servicios durante la ejecuci\'on del sistema.
...
un \texttt{timestamp} preformateado como texto. Un arn\'es ejecutable
(\path{experimentos/arnes_12_vectores.py}, ver
\path{experimentos/README_arnes.md}) compila e invoca las tres
implementaciones reales sobre doce vectores independientes: las dos
implementaciones Java coinciden en los doce casos (12/12, 100.0\%),
mientras que la implementaci\'on Python coincide con Java en ocho de
los doce (8/12, 66.7\%). Los cuatro casos en que las representaciones
difieren son:

\begin{tabular}{l|l|l}
\hline
Tipo de valor & Representaci\'on Java & Representaci\'on Python \\
\hline
Flotante de rango extremo peque\~no & \texttt{1.0E-7} & \texttt{1e-07} \\
Flotante de rango extremo grande & \texttt{1.0E21} & \texttt{1e+21} \\
Orden de claves suplementarias & U+1F600 antes de U+FF01 (UTF-16) & orden por punto de c\'odigo \\
\texttt{NaN} & cadena \texttt{"NaN"} & literal JSON no est\'andar \texttt{NaN} \\
\hline
\end{tabular}

\paragraph{Qu\'e protege el vector patr\'on, qu\'e no protege.}
Protege: cadenas ASCII, enteros dentro del rango que las pruebas cubren,
\texttt{timestamp} preformateado como cadena, claves ordenadas por punto
de c\'odigo Unicode dentro del plano b\'asico y exclusi\'on de las claves
sensibles enumeradas (\texttt{authorization}, \texttt{jwt},
\texttt{password}, \texttt{contrasena}, \texttt{contrase\~na},
\texttt{contrase?a}, \texttt{internal\_token}, \texttt{token},
\texttt{secret}). No protege: representaci\'on de flotantes de rango
extremo, marcas de tiempo como \texttt{Instant} sin preformatear ---en
producci\'on Docente serializa \texttt{datetime} con offset
(\texttt{2026-09-21T20:00:00+00:00}) y Principal serializa
\texttt{Instant} con sufijo UTC (\texttt{2026-09-21T20:00:00Z})---,
orden de claves con caracteres suplementarios (U+10000 y siguientes), el
valor especial \texttt{NaN} ni ning\'un tipo fuera del vector patr\'on.

\paragraph{Interoperabilidad real entre lenguajes.}
Los verificadores rehashean el texto almacenado en la columna
\texttt{contenido\_canonico}, no exigen que cada implementaci\'on
produzca el mismo texto para el mismo evento. En consecuencia, la
propiedad verificada es \textit{la integridad de la cadena tal como fue
almacenada}, no la equivalencia de las representaciones can\'onicas
entre lenguajes. El cotejo independiente de los doce vectores vive de
forma ejecutable en \path{experimentos/arnes_12_vectores.py}: si Java
Principal deja de coincidir con Java Secretaria en alg\'un vector, o si
el conteo Java == Python cambia respecto a la evidencia congelada
(8/12), el arn\'es falla con c\'odigo de salida 1.
...
car\'acter por car\'acter entre lenguajes. Las cuatro divergencias
medidas (m\'as la divergencia de tipos de marca de tiempo fuera del
vector patr\'on) constituyen la superficie residual de riesgo y se
documentan en el apartado de amenazas a la validez.
...
\begin{enumerate}
  \item \textbf{Resoluci\'on temporal y entorno de ejecuci\'on:} Las 160 observaciones de latencia se conservan completas y se analizan como 16 celdas independientes ($4$ mecanismos $\times$ $4$ niveles, $n=10$ por celda), sin mezclar los niveles 1, 5, 10 y 14. Para evitar asimetr\'ias de instrumentaci\'on y cuantizaci\'on en operaciones breves, las latencias locales de los cuatro mecanismos $M_0$, $M_1$, $M_2$ y $M_3$ se miden con el mismo procedimiento por lotes amortizados. Cada lote usa una sola pareja de lecturas de \texttt{perf\_counter\_ns}; su ventana observada supera al menos 1 ms y 10\,000 pasos de la resoluci\'on declarada por \texttt{perf\_counter}, y el tiempo total se divide por el n\'umero de operaciones ejecutadas. De este modo, las diferencias de mediana comparan mecanismos bajo el mismo coste de temporizaci\'on. No se introducen pausas, ruido sint\'etico ni valores fabricados. El bloque sigue siendo un microbenchmark local secuencial en memoria, no una prueba de concurrencia HTTP ni de acceso real a base de datos; la planificaci\'on del sistema operativo permanece como amenaza residual.
...
en Python) sin biblioteca compartida. Sobre los doce vectores
independientes del arn\'es \path{experimentos/arnes_12_vectores.py}, las
dos implementaciones Java coinciden en el 100\% de los casos (12/12,
100.0\%) y la implementaci\'on Python coincide con Java en ocho de los
doce (8/12, 66.7\%). Las cuatro divergencias medidas son: flotantes de
rango extremo peque\~no (\texttt{1.0E-7} frente a \texttt{1e-07}),
flotantes de rango extremo grande (\texttt{1.0E21} frente a
\texttt{1e+21}), orden de las claves suplementarias (UTF-16 frente a
puntos de c\'odigo Unicode) y el valor especial \texttt{NaN} (cadena
\texttt{"NaN"} en Java frente al literal \texttt{NaN} en Python). Existe
adem\'as una amenaza residual sobre los tipos de producci\'on: los
vectores entregan la marca de tiempo preformateada como cadena, pero
cuando los servicios la manejan como objeto nativo, Python produce
formato ISO con offset (\texttt{2026-09-21T20:00:00+00:00}) y Java
produce sufijo UTC (\texttt{2026-09-21T20:00:00Z}); por ello la cadena
se valida sobre el rehash del texto can\'onico persistido y sobre el
vector patr\'on institucional, lo que justifica la decisi\'on de no
forzar una biblioteca com\'un pol\'iglota. La extracci\'on de una
biblioteca can\'onica compartida queda como trabajo futuro.
\end{enumerate}
...
El desarrollo se ejecutó en ramas de trabajo (\texttt{Ernesto-Luna}, \texttt{devBedon}, \texttt{Juliana-Emanuel}, \texttt{Leonardo-Castro}, \texttt{feat/cluster-db-fragmentacion} y ramas puntuales \texttt{fix/*}) y se integró mediante PR. El historial consultado no acredita que cada requerimiento, módulo o corrección tuviera previamente un issue con etiquetas, responsable y criterios de aceptación; esa trazabilidad debe comprobarse caso por caso.
