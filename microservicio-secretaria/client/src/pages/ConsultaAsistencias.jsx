...
export default function ConsultaAsistencias() {
  ...
  const [asistenciasPorAsignacion, setAsistenciasPorAsignacion] = useState({});
  const [loading, setLoading] = useState(true);
  const [modalSesion, setModalSesion] = useState(null);

  // 1. CARGAR DATOS INICIALES (grados/matriculas/asignaciones vienen de sga-principal)
  useEffect(() => {
    setLoading(true);
    Promise.all([
      apiPrincipal.get("/grados").catch(() => ({ data: [] })),
      apiPrincipal.get("/matriculas", { params: { limit: 300 } }).catch(() => ({ data: { items: [] } })),
      apiPrincipal.get("/asignaciones").catch(() => ({ data: [] })),
    ])
  ...
  // 2. AL CAMBIAR EL GRADO: FILTRAR ALUMNOS Y MATERIAS ASIGNADAS DE ESE CURSO
  const actualizarGrado = useCallback(() => {
    ...
    const mapeadas = asigGrado.map((a, idx) => ({
      ...
      materiaNombre: a.asignatura || a.nombreAsignatura || `Asignatura #${idx + 1}`,
      codigoMateria: `EGB-${100 + idx}`,
      docenteNombre: a.docente || "Docente Asignado",
  ...
  // 3. CONSULTAR ASISTENCIAS REALES (endpoint propio de Secretaria, lectura SQL directa)
  useEffect(() => {
    if (materiasDelGrado.length === 0) return;

    materiasDelGrado.forEach((mat) => {
      api.get(`/asistencias/asignacion/${mat.idAsignacion}`)
        .then((r) => {
          setAsistenciasPorAsignacion((prev) => ({ ...prev, [mat.idAsignacion]: r.data?.asistencias || [] }));
        })
        .catch(() => {
          setAsistenciasPorAsignacion((prev) => ({ ...prev, [mat.idAsignacion]: [] }));
        });
    });
  }, [materiasDelGrado]);
  ...
  const getNombreAlumno = (m) => {
    ...
    if (m.estudianteNombres || m.estudianteApellidos) {
      return `${m.estudianteApellidos || ""} ${m.estudianteNombres || ""}`.trim();
    }
    return `Estudiante #${m.idMatricula}`;
  };

  const getCedulaAlumno = (m) => {
    if (!m) return "—";
    return m.estudianteCedula || "—";
  };
  ...
  return (
    ...
    >
      {/* BARRA SUPERIOR DE SELECCIÓN DE CURSO */}
      ...
      {seccion === "materia" && (
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
          <div style={{ backgroundColor: PRIMARY }} className="text-white px-5 py-3 text-xs font-bold uppercase tracking-wider grid grid-cols-12 gap-3 items-center">
            <div className="col-span-12 md:col-span-4">MATERIA / DOCENTE</div>
            <div className="col-span-6 md:col-span-2 text-center">% ASISTENCIA</div>
            <div className="col-span-6 md:col-span-6 text-right">REGISTRO DIARIO DE CLASES</div>
          </div>

          <div className="divide-y divide-slate-100">
            {materiasDelGrado.length === 0 && (
              <div className="p-8 text-center text-slate-400 text-xs">
                Este curso no tiene materias asignadas todavía.
              </div>
            )}
            {materiasDelGrado.map((mat) => {
              const asistList = asistenciasPorAsignacion[mat.idAsignacion] || [];
              const totalSes = asistList.length;
              const ausentes = asistList.filter((a) => a.estado === "AUSENTE").length;
                ...
                <div key={mat.idAsignacion} className="p-4 grid grid-cols-12 gap-4 items-center hover:bg-slate-50/80 transition">
                  <div className="col-span-12 md:col-span-4 space-y-1.5">
                    <h3 className="text-xs font-bold text-slate-800 leading-snug">
                      {mat.materiaNombre} - A - {mat.gradoNombre.toUpperCase()}
                    </h3>
                  ...
                  <div className="col-span-12 md:col-span-6">
                    <div className="flex flex-wrap gap-1 items-center max-h-24 overflow-y-auto p-1">
                      {asistList.map((s, idx) => {
                        const esFalta = s.estado === "AUSENTE";
      ...
      {/* VISTA 2: ASISTENCIA INDIVIDUAL POR ESTUDIANTE */}
      {seccion === "estudiantes" && (
          ...
          <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm md:col-span-1">
            ...
            {estudiantesMatriculados.length === 0 ? (
              <div className="p-6 text-center text-slate-400 text-xs">
                Este curso no registra estudiantes matriculados.
              </div>
            ) : (
            ...
            {estudianteSel ? (
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  {materiasDelGrado.map((mat) => {
                    const asistList = (asistenciasPorAsignacion[mat.idAsignacion] || [])
                      .filter((a) => String(a.id_matricula) === String(estudianteSel.idMatricula));
                    const totalSes = asistList.length;
                    const ausentes = asistList.filter((a) => a.estado === "AUSENTE").length;
          ...
          <div className="bg-white rounded-2xl max-w-md w-full overflow-hidden shadow-2xl border border-slate-200">
            ...
            <div className="p-5 space-y-3 text-xs">
              <div className="grid grid-cols-2 gap-2 bg-slate-50 p-3 rounded-lg border border-slate-200 font-mono">
                ...
                <div>
                  <span className="text-slate-400 block text-[10px] uppercase font-sans">Justificación</span>
                  <span className="font-bold text-slate-700">{modalSesion.sesion.justificacion || "—"}</span>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between pt-2">
                ...
                <span className={`px-3 py-1 rounded-full text-white font-extrabold ${modalSesion.sesion.estado === "AUSENTE" ? "bg-rose-600" : "bg-emerald-600"}`}>
                  {modalSesion.sesion.estado === "AUSENTE" ? "✖ FALTA REGISTRADA" : "✔ PRESENTE"}
                </span>
              </div>
            </div>
