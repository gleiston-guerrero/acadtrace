...
export default function Asignaciones() {
  ...
  const ALIASES = {
    lenguaje: "lengua", literatura: "lengua",
    mate: "matemática", matematica: "matemática", matematicas: "matemática",
    ciencias: "ciencias", naturales: "ciencias", cn: "ciencias",
    sociales: "estudios", estudios: "estudios", "ee.ss": "estudios",
    fisica: "física", deporte: "física", ef: "física",
    eca: "cultural", arte: "cultural", cultural: "cultural",
    ingles: "inglés", english: "inglés",
    lectura: "lectura", animacion: "lectura",
    tutor: "acompañamiento", "acompañamiento": "acompañamiento",
    curriculo: "currículo", integrado: "currículo",
  };
  ...
  const opcionesAsignaturas = (() => {
    ...
    (asignaturas || []).forEach(a => {
      if (!mapa.has(Number(a.idAsignatura))) mapa.set(Number(a.idAsignatura), a.nombre);
    });
  ...
  useEffect(() => {
    cargar();
    apiPrincipal.get("/asignaturas").then(r => setAsignaturas(r.data)).catch(() => {});
    apiPrincipal.get("/grados").then(r => setGrados(r.data)).catch(() => {});
    apiPrincipal.get("/anos-lectivos/actual").then(r => {
      setAnoActual(r.data);
      setForm(f => ({ ...f, idAnoLectivo: r.data?.idAnoLectivo || "" }));
    }).catch(() => {});
  ...
  useEffect(() => {
    if (form.idGrado) {
      apiPrincipal.get(`/asignaciones/grado/${form.idGrado}/paralelos`)
  ...
  useEffect(() => {
    if (form.idGrado && form.idAnoLectivo) {
      apiPrincipal.get(`/malla/grado/${form.idGrado}`, { params: { idAnoLectivo: form.idAnoLectivo } })
  ...
  const cargarMateriasMallaEdit = (idGrado, idAnoLectivo) => {
    ...
    apiPrincipal.get(`/malla/grado/${idGrado}`, { params: { idAnoLectivo } })
  ...
  const handleSeccion = (id) => {
    ...
    setError("");
    if (id === "nuevo") {
      setForm({ ...formVacio, idAnoLectivo: anoActual?.idAnoLectivo || "" });
      setCedulaInput("");
      setDocenteSel(null);
      setNombreQuery("");
  ...
  const buscarDocentePorCedula = async () => {
    ...
    setBuscando(true);
    try {
      const { data } = await apiPrincipal.get("/personas/buscar", { params: { cedula: cedulaInput } });
  ...
  const eliminarAsignacion = async (id, nombreMateria) => {
    ...
    try {
      await apiPrincipal.delete(`/asignaciones/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      await apiPrincipal.post("/asignaciones", {
  ...
  const toggleEstado = async (a) => {
    try {
      await apiPrincipal.patch(`/asignaciones/${a.idAsignacion}/estado?activo=${!a.activo}`);
      cargar();
    } catch {
  ...
  const abrirEditar = (a) => {
    ...
    if (a.idGrado) {
      apiPrincipal.get(`/asignaciones/grado/${a.idGrado}/paralelos`)
        .then(r => setParalelos(r.data)).catch(() => setParalelos([]));
    }
  ...
  const guardarEdicionAsign = async (e) => {
    ...
    try {
      await apiPrincipal.put(`/asignaciones/${asignEdit.idAsignacion}`, {
                  ...
                  <div className={`${currentPalette.header} px-4 py-3 text-white flex justify-between items-center relative`}>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="relative">
                        <button type="button" onClick={() => setActiveMenuKey(isMenuOpen ? null : curso)}
                        ...
                        {isMenuOpen && (
                          <div className="absolute right-0 top-7 w-40 bg-white rounded-lg shadow-xl border border-slate-200 py-1 z-30 text-slate-700">
                            <button type="button" onClick={cambiarColorCard}
      ...
      {seccion === "nuevo" && (
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
              ...
              {modoBusqueda === "cedula" ? (
                <>
                  ...
                  <div className="flex gap-2">
                    <input type="text" inputMode="numeric" maxLength={10} value={cedulaInput}
                    ...
                    <button type="button" onClick={buscarDocentePorCedula} disabled={buscando || cedulaInput.length !== 10}
                ...
                <div>
                  ...
                  <input type="text" value={nombreQuery}
            ...
            <div className="lg:col-span-2 lg:border-l lg:border-slate-100 lg:pl-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div className="md:col-span-2">
                  ...
                  <input disabled value={anoActual?.nombre || "Cargando..."}
                    className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm bg-slate-100 text-slate-500" />
                </div>
                <div className="md:col-span-2">
                  ...
                  <div className="mb-2">
                    <input type="text" value={filtroAsignatura} onChange={(e) => setFiltroAsignatura(e.target.value)}
                ...
                <div>
                  ...
                  <input type="number" min={1} max={40} required value={form.horasSemanales}
          ...
          <div className="px-6 lg:px-8 py-4 bg-slate-50 border-t border-slate-100 flex gap-3 justify-end">
            <button type="button" onClick={() => { setForm({ ...formVacio, idAnoLectivo: anoActual?.idAnoLectivo || "" }); setCedulaInput(""); setDocenteSel(null); setSeccion("lista"); }}
        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          <div className="p-4 border-b border-slate-100 flex justify-between items-center gap-3 flex-wrap">
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => navigate("/usuarios")} style={{ backgroundColor: PRIMARY }}
                ...
                {personasDocentes.map((p) => (
                  <tr key={p.idPersona} className="border-t border-slate-100 hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-center">
                      <button onClick={() => setDocenteVer(p)}
            ...
            <div className="p-6 space-y-4 max-h-[75vh] overflow-y-auto">
              <div className="flex items-center gap-4 pb-3 border-b border-slate-100">
                <div className="w-16 h-16 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center overflow-hidden flex-shrink-0">
                  {docenteVer.fotoUrl
                    ? <img src={docenteVer.fotoUrl.startsWith("http") ? docenteVer.fotoUrl : `http://${window.location.hostname}:8080${docenteVer.fotoUrl}`} alt="" className="w-full h-full object-cover" />
              ...
              <div className="flex items-center gap-4 pb-3 border-b border-slate-100">
                <div className="w-14 h-14 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center overflow-hidden">
                  {asignVer.fotoDocente ? (
                    <img src={asignVer.fotoDocente.startsWith("http") ? asignVer.fotoDocente : `http://${window.location.hostname}:8080${asignVer.fotoDocente}`} alt="" className="w-full h-full object-cover" />
                  ) : (
            ...
            <form onSubmit={guardarEdicionAsign} className="p-6 space-y-4">
              ...
              <div>
                ...
                <select required value={asignEdit.idDocente} onChange={(e) => setAsignEdit({ ...asignEdit, idDocente: e.target.value })}
                  ...
                  {personasDocentes.map((p) => (
                    <option key={p.idPersona} value={p.idPersona}>{p.nombres} {p.apellidos} ({p.cedula || p.username})</option>
                  ))}
                </select>
              </div>
                ...
                <div>
                  ...
                  <select required value={asignEdit.idGrado}
                    onChange={(e) => {
                      ...
                      if (g) apiPrincipal.get(`/asignaciones/grado/${g}/paralelos`).then(r => setParalelos(r.data)).catch(() => setParalelos([]));
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-500 mb-1 uppercase">Año lectivo</label>
                <input disabled value={anoActual?.nombre || ""}
              ...
              <div>
                ...
                <input type="number" min={1} max={40} required value={asignEdit.horasSemanales || 4}
