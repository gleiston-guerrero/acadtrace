...
class NotificationPayloadTest {
    ...
    @Test fun ausenciaNavegaAAsistencia() = assertEquals(Screen.Asistencia.create(12), NotificationDestination.route(mapOf("type" to "AUSENCIA", "studentId" to "12")))
    ...
    @Test fun incompletoODesconocidoNoNavega() {
        assertNull(NotificationDestination.route(mapOf("type" to "AUSENCIA")))
    ...
    @Test fun sesionVencidaSiempreAbreLogin() = assertEquals(
        Screen.Login.route,
        NotificationDestination.startRoute(false, Screen.Home.route, mapOf("type" to "AUSENCIA", "studentId" to "12"))
    )
